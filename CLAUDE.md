...
**The design system is a shared, published package — `saiku-ui/design-system/`, an npm workspace of `saiku-ui`, published as `@concepttocloud/saiku-design-system` (Apache-2.0).** It carries **its own version** and is published by its **own workflow** (`.github/workflows/design-system.yml`, triggered by changes under `saiku-ui/design-system/**`), NOT by `release.yml` — it moves on its own cadence so saiku-cloud isn't waiting on a product release for a token change. Bump `version` in the package's `package.json` and merge to release; the workflow checks the registry first and no-ops if that version is already out. It owns the token layer (`tokens.css`), the Tailwind `@theme inline` bridge (`tailwind.css`), the shadcn-style primitives (Button/Card/Input/Tooltip) and the compounds (PageHeader, FeedbackBanner, Badge, EmptyState, SectionCard, Toast, FormField, SortableColumnHeader). **The Saiku Cloud dashboard consumes the same package from npm**, so a change here ships to both products — that's the point, and it's also the hazard. `src/lib/design-system/` and `src/lib/components/ui/` are now thin barrels re-exporting the package plus the Storybook stories; import from the barrels (`$lib/design-system`, `$lib/components/ui`), never deep paths.

It is a **workspace, not a `file:` dep, and that's load-bearing** — a `file:` dep gives the package its own nested `node_modules/svelte`, and two Svelte installs make `Snippet` from the package structurally incompatible with `Snippet` in the app. The symptom is a wave of "Two different types with this name exist, but they are unrelated" errors in files that have nothing to do with the design system. `npm ci` runs the package's `prepare` (svelte-package) automatically, so `dist/` exists before the app builds; after editing package source run `npm run build --workspace @concepttocloud/saiku-design-system`.

The package's `tailwind.css` deliberately **omits preflight** (saiku-ui's legacy base CSS in `app.css` depends on browser defaults). saiku-cloud, which has always run with preflight, opts back in explicitly. Don't "fix" this by adding preflight to the package.
...
- **Reactor compiler pinned to JDK 6 source in root pom** but `saiku-launcher` overrides to `<release>21</release>`. Don't "fix" the root pom — modules that need 21 set it locally. `saiku-proptest` follows the same pattern with `<configuration combine.self="override"><release>22</release>` (Hegel needs 22); the `combine.self="override"` is load-bearing — without it the inherited `source/target 1.6` collides with the bare `<release>` and the compiler plugin errors.
- **`saiku-proptest`** is a test-only module (no main sources, never published) holding Hegel property-based tests against real Saiku code (`JdbcUrlValidator`, `CryptoUtil`, …). It depends on `saiku-web`/`saiku-service` in test scope, needs JDK 22, and its surefire sets `--enable-native-access=ALL-UNNAMED` for Hegel's native engine. See `saiku-proptest/README.md`.

<script lang="ts">
	...
	function sumColumn(rs: EmbedRow[], col: string): number {
		return rs.reduce(
			(a, r) => a + (typeof r[col]?.value === 'number' ? (r[col].value as number) : 0),
			0
		);
	}
	...
	const kpi = $derived.by(() => {
		...
		const display = singleRow
			? (rows[0][measureCol]?.formatted ?? compact.format(value))
...
{:else}
	<div class="kpi" role="figure" aria-label={kpi.label}>
		<div class="cap">
			{kpi.label}{#if kpi.unit}<span class="unit"> · {kpi.unit}</span>{/if}
		</div>

...
describe('fetchSavedQuery', () => {
	...
	it("throws EmbedFetchError carrying the server's status + body on 4xx", async () => {
		...
		await expect(
			fetchSavedQuery('https://demo.saiku.bi', 'homes/admin/x.saiku', 'tok')
	...
	it('POSTs a {filters} body when embed-time filters are supplied', async () => {
		...
		expect(calls[0].url).toBe(
			'https://demo.saiku.bi/rest/saiku/api/embed/query/homes/admin/x.saiku'
		);
	});
...
describe('askEmbedAi', () => {
	...
	it('POSTs the question to the pinned cube ask URL, no space by default', async () => {
		...
		expect(calls[0].url).toBe(
			'https://demo.saiku.bi/rest/saiku/api/embed/ai/conn/cat/sch/Sales/ask'
		);

<script lang="ts">
  ...
  function tileFetch(tileId: string, overrides: EmbedFilterOverride[]) {
    return fetchDashboardTile(server.trim(), path.trim(), tileId, token.trim() || undefined, overrides);
  }
  function memberFetch(tileId: string, q?: string, limit?: number) {
    return fetchTileMembers(server.trim(), path.trim(), tileId, token.trim() || undefined, q, limit);
  }
</script>

<script lang="ts">
	...
	import { createSchemaGenClient, type SuggestionOp } from '$lib/api/schemaGen';
	...
	const effectiveStage = $derived(store.sessionId === null ? null : store.stage);
	...
	function handleEdit(
		...
		if (field === 'caption' || field === 'description') {
			...
			const description = field === 'description' ? value : (node.description ?? null);
			if (
...
<div class="schemagen">
	<header
		class="flex items-center justify-between gap-4 border-b border-border bg-bg-muted px-6 py-3"
	>
		...
		<div class="schemagen__actions">
			...
			<button type="button" onclick={handleStart} disabled={startDisabled}> Start </button>
			<button type="button" onclick={handleSave} disabled={saveDisabled}> Save </button>
			{#if cancelVisible}
				<button type="button" class="text-fg-muted" onclick={handleCancel}> Cancel </button>
			{/if}
		</div>
	</header>
	...
	<main class="schemagen__main">
		...
		>
			{#if selectedPath !== null}
				...
				<NodeDrawer draft={store.draft} {selectedPath} onEdit={handleEdit} />
			{/if}
		</aside>
	</main>
</div>

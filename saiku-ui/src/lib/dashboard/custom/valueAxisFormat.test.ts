...
describe('applyValueAxisFormat', () => {
	it('formats a value axis', () => {
		...
		const fmt = (option.yAxis as Record<string, Record<string, unknown>>).axisLabel.formatter as (
			v: number
		) => string;
	...
	it('leaves a category axis alone', () => {
		const option = { xAxis: { type: 'category' }, yAxis: { type: 'value' } } as Record<
			string,
			unknown
		>;
		applyValueAxisFormat(option, '$c0');
	...
	it('formats a value axis declared on x (horizontal bars)', () => {
		const option = { xAxis: { type: 'value' }, yAxis: { type: 'category' } } as Record<
			string,
			unknown
		>;
		applyValueAxisFormat(option, '$c0');
		expect(
			typeof (option.xAxis as Record<string, Record<string, unknown>>).axisLabel.formatter
		).toBe('function');
	});

...
describe('ancestorSelections', () => {
	...
	test('returns empty list when schema is null', () => {
		expect(
			ancestorSelections([active('Country', ['[X]'])], null, SALES, childTarget('State Province'))
		).toEqual([]);
	});

	test("returns empty list when child level isn't in the schema", () => {
		expect(
			ancestorSelections([active('Country', ['[X]'])], customerSchema(), SALES, {
				dimension: 'Customer',
				hierarchy: 'Customers',
				level: 'Made Up',
				members: []
...
describe('memberDescendsFromAny', () => {
	test('matches descendant via prefix', () => {
		expect(
			memberDescendsFromAny('[Customer].[Customers].[USA].[OR]', ['[Customer].[Customers].[USA]'])
		).toBe(true);
	});

	test('rejects when member is the parent itself', () => {
		expect(
			memberDescendsFromAny('[Customer].[Customers].[USA]', ['[Customer].[Customers].[USA]'])
		).toBe(false);
	});

	test('rejects unrelated member', () => {
		expect(
			memberDescendsFromAny('[Customer].[Customers].[Mexico].[DF]', [
				'[Customer].[Customers].[USA]'
			])
		).toBe(false);
	});
	...
	test('multi-parent OR semantics — descends if any parent matches', () => {
		expect(
			memberDescendsFromAny('[Customer].[Customers].[USA].[OR]', [
	...
	test('guards against the prefix-confusion case', () => {
		...
		expect(
			memberDescendsFromAny('[Customer].[Customers].[USA].[OR2]', [
				'[Customer].[Customers].[USA].[OR]'
			])
		).toBe(false);
	});

...
describe('kpiThresholdToken', () => {
	...
	it('lower-is-better: small values are green', () => {
		expect(kpiThresholdToken(5, { red: 100, yellow: 50, green: 10 }, 'lower-is-better')).toBe(
			'--kpi-green'
		);
	});

	it('lower-is-better: large values are red', () => {
		expect(kpiThresholdToken(150, { red: 100, yellow: 50, green: 10 }, 'lower-is-better')).toBe(
			'--kpi-red'
		);
	});
});
...
describe('lastAndPriorValues', () => {
	...
	it('returns the last and second-to-last non-null values', () => {
		expect(lastAndPriorValues([{ value: 10 }, { value: 20 }, { value: 30 }])).toEqual({
			current: 30,
			prior: 20
		});
	});
	...
	it("skips null cells so a blank row doesn't shift the baseline", () => {
		expect(lastAndPriorValues([{ value: 10 }, { value: null }, { value: 30 }])).toEqual({
			current: 30,
			prior: 10
		});
	});
	...
	it('returns null for both when every cell is null', () => {
		expect(lastAndPriorValues([{ value: null }, { value: null }])).toEqual({
			current: null,
			prior: null
		});
	});

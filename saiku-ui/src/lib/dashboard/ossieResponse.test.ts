...
function realSuccess() {
	return {
		...
		records: [
			{
				'carrier.carrier_name': 'American Airlines',
				flight_count: { value: 18.0, formatted: '18' }
			},
			{ 'carrier.carrier_name': 'Delta Air Lines', flight_count: { value: 18.0, formatted: '18' } },
			{
				'carrier.carrier_name': 'Southwest Airlines',
				flight_count: { value: 12.0, formatted: '12' }
			}
		],
...
describe('toAiQueryResponse — the last step before the canvas', () => {
	it('builds a complete ECharts option from the real payload', () => {
		...
		const opt = buildChartOption(toAiQueryResponse(realSuccess()), 'bar') as Record<
			string,
			unknown
		>;

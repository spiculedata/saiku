...
describe('app context scope (saiku#1754)', () => {
	...
	test('omits the param when there is no selection', () => {
		expect(new URLSearchParams(encodeAppFilterState('page-1', {}, null)).has('ctx')).toBe(false);
		expect(new URLSearchParams(encodeAppFilterState('page-1', {}, '')).has('ctx')).toBe(false);
	});

	test('round-trips a label containing separators', () => {
		const label = 'All regions · National';
		const decoded = decodeAppFilterState(new URLSearchParams(encodeAppFilterState('p', {}, label)));
		expect(decoded.contextLabel).toBe(label);
	});

	test('decodes to null when absent', () => {
		expect(decodeAppFilterState(new URLSearchParams('?p=page-1')).contextLabel).toBeNull();
	});

...
function makeCell(v: string | number): {
...
describe('pivotResult', () => {
	...
	test('basic 1 row × 1 col × 1 value crosstab', () => {
		...
		expect(grid!.headerRows[0].map((c) => c.formatted)).toEqual([
			'product.brand',
			'Commercial',
			'Medicare'
		]);
	...
	test('multi-value shelves get a metric-name header row + colspans on column labels', () => {
		...
		const grid = pivotResult(['product.brand'], ['payer.channel'], ['net_revenue', 'rx_count'], r)!;

<script lang="ts">
	...
	let { defaultName, defaultFolder, open, importing = false, onCreate, onCancel }: Props = $props();
...
<Modal title={importing ? 'New app from dashboard' : 'New app'} {open} size="lg" onClose={onCancel}>
	{#if importing}
		<p class="import-badge">
			This app will wrap the selected dashboard's layout as its first page. You can add more pages
			after it opens.
		</p>
	{/if}
	...
	<label class="field">
		...
		<input class="field__input" bind:value={name} placeholder="Untitled app" autocomplete="off" />
	</label>

<script lang="ts">
	...
	const LEGEND_POSITIONS: ChartOptions['legendPosition'][] = ['top', 'bottom', 'left', 'right'];

	// issue #1071: map colour ramps (must mirror COLOR_RAMPS in charts/build.ts).
	const COLOR_RAMP_IDS: ChartColorRamp[] = [
		'theme',
		'blues',
		'greens',
		'reds',
	...
	function cloneForm(src: ChartOptions): ChartOptions {
		return {
			...
			conditionalFormat: (src.conditionalFormat ?? []).map((c) => ({
				...
				rules: c.rules.map((r) => ({
					...r,
					value: Array.isArray(r.value) ? ([...r.value] as [number, number]) : r.value
				}))
			}))
		};
	}
	...
	function updateRefLine(idx: number, patch: Partial<ReferenceLine>): void {
		form.referenceLines = (form.referenceLines ?? []).map((l, i) =>
			i === idx ? { ...l, ...patch } : l
		);
	}
	...
	function updateRefBand(idx: number, patch: Partial<ReferenceBand>): void {
		form.referenceBands = (form.referenceBands ?? []).map((b, i) =>
			i === idx ? { ...b, ...patch } : b
		);
	}
	...
	const showCondFormat = $derived(
		chartType !== undefined &&
			CONDITIONAL_FORMAT_CHART_TYPES.has(chartType) &&
	...
	function addCondRule(mi: number): void {
		editBand(mi, (b) => ({
			...b,
			rules: [...b.rules, { op: 'gt', value: 0, color: COND_DEFAULT_COLOR }]
		}));
	}
	...
	function updateCondRule(mi: number, ri: number, patch: Partial<ChartCondRule>): void {
		editBand(mi, (b) => ({
			...b,
			rules: b.rules.map((r, i) => (i === ri ? { ...r, ...patch } : r))
		}));
	}
	...
	<div class="flex flex-col gap-3">
		<label class="field">
			...
			<input
				...
				placeholder={i18n.t('modal.chart.chartTitlePlaceholder')}
			/>
		...
		{#if chartType === 'map'}
           ...
           hierarchy; the active (first) measure drives the colour. -->
			<div class="map-opts">
				...
				<p class="m-0 text-xs text-fg-subtle">{i18n.t('modal.chart.map.hint')}</p>
			</div>
		{/if}
		...
		{#if chartType !== 'map'}
			...
			<div class="flex gap-3">
				<label class="field flex-1">
					...
					<label class="inline-flex items-center gap-2 text-fg">
						<input type="checkbox" bind:checked={form.showLegend} />
						{i18n.t('modal.chart.showLegend')}
					</label>
				</label>
				<label class="field flex-1">
			...
			<div class="colours">
				...
				{#if seriesNames.length > 0}
					<p class="m-0 text-xs text-fg-subtle">{i18n.t('modal.chart.seriesColors.hint')}</p>
					<div class="flex flex-col gap-2">
						{#each seriesNames as name (name)}
							<div class="flex items-center gap-3">
								<span
									class="flex-1 overflow-hidden text-sm text-ellipsis whitespace-nowrap text-fg"
			...
			<div class="flex gap-3">
				<label class="field flex-1">
					...
					<label
						...
						title={i18n.t('modal.chart.hideRollupRows.hint')}
					>
						<input type="checkbox" bind:checked={form.hideRollupRows} />
						{i18n.t('modal.chart.hideRollupRows')}
					</label>
				</label>
			</div>
			...
			{#if seriesNames.length >= 2}
				<div class="flex gap-3">
					<label class="field flex-1">
						...
						<label
							class="inline-flex items-center gap-2 text-fg"
							title={i18n.t('modal.chart.dualAxis.hint')}
						>
							<input type="checkbox" bind:checked={form.dualAxis} />
							{i18n.t('modal.chart.dualAxis')}
						</label>
					</label>
				</div>
			{/if}
         ...
         so the section is hidden rather than dangled as a useless picker. -->
			{#if seriesNames.length >= 2}
				<div class="series-axis">
					...
					<p class="m-0 text-xs text-fg-subtle">{i18n.t('modal.chart.seriesAxis.hint')}</p>
					<div class="flex flex-col gap-2">
						...
						{#each seriesNames as name, ni (ni)}
							<div class="flex items-center gap-3">
								...
								<select
									...
									onchange={(e) =>
										setAxisPick(name, (e.currentTarget as HTMLSelectElement).value as AxisPick)}
								>
         ...
         multi-series only — pick a type per series (or "Default" = chart type). -->
			{#if showComboTypes}
				<div class="series-axis">
					<span class="text-sm text-fg-muted"
						>{i18n.t('modal.chart.seriesType', 'Series type (combo)')}</span
					>
					<p class="m-0 text-xs text-fg-subtle">
						{i18n.t(
							...
							<div class="flex items-center gap-3">
								...
								<select
									...
									onchange={(e) =>
										setSeriesType(
											name,
											(e.currentTarget as HTMLSelectElement).value as ComboSeriesType | 'default'
										)}
								>
									<option value="default"
										>{i18n.t('modal.chart.seriesType.default', 'Default')}</option
									>
			...
			<p class="m-0 text-xs text-fg-subtle">{i18n.t('modal.chart.trendHint')}</p>
         ...
         values, data labels). All controls optional; blank/off = raw values. -->
			<div class="number-format">
				...
				<div class="flex gap-3">
					<label class="field flex-1">
						...
						<input
							...
							oninput={(e) =>
								setNumberFormat({ prefix: (e.currentTarget as HTMLInputElement).value })}
						/>
					</label>
					<label class="field flex-1">
						...
						<input
							...
							oninput={(e) =>
								setNumberFormat({ suffix: (e.currentTarget as HTMLInputElement).value })}
						/>
					</label>
					<label class="field flex-1">
				...
				<div class="flex gap-3">
					<label class="field flex-1">
						<label class="inline-flex items-center gap-2 text-fg">
							<input
								...
								checked={nfThousands}
								onchange={(e) =>
									setNumberFormat({ thousands: (e.currentTarget as HTMLInputElement).checked })}
							/>
					...
					<label class="field flex-1">
						<label
							class="inline-flex items-center gap-2 text-fg"
							title={i18n.t('modal.chart.numberFormat.abbreviate.hint')}
						>
							<input
								type="checkbox"
				...
				<p class="m-0 text-xs text-fg-subtle">{i18n.t('modal.chart.numberFormat.hint')}</p>
			</div>

			{#if showRefSection}
				<div class="ref">
					<span class="text-sm text-fg-muted">{i18n.t('modal.chart.refLines')}</span>
					<p class="m-0 text-xs text-fg-subtle">{i18n.t('modal.chart.refLines.hint')}</p>
					<div class="flex flex-col gap-2">
						{#each form.referenceLines ?? [] as line, i (i)}
							<div class="flex items-center gap-2">
								...
								<input
									...
									value={line.label ?? ''}
									oninput={(e) =>
										updateRefLine(i, { label: (e.currentTarget as HTMLInputElement).value })}
								...
								<input
									...
									oninput={(e) =>
										updateRefLine(i, { color: (e.currentTarget as HTMLInputElement).value })}
					...
					<Button variant="outline" size="sm" class="ref__add" onclick={addRefLine}
						>{i18n.t('modal.chart.refLines.add')}</Button
					>

					<span class="text-sm text-fg-muted">{i18n.t('modal.chart.refBands')}</span>
					<p class="m-0 text-xs text-fg-subtle">{i18n.t('modal.chart.refBands.hint')}</p>
					<div class="flex flex-col gap-2">
						{#each form.referenceBands ?? [] as band, i (i)}
							<div class="flex items-center gap-2">
								...
								<input
									...
									oninput={(e) =>
										updateRefBand(i, { color: (e.currentTarget as HTMLInputElement).value })}
									aria-label={i18n.t('modal.chart.refLines.color')}
									title={i18n.t('modal.chart.refLines.color')}
								/>
								<Button
									...
									size="sm"
									class="ref__remove"
									onclick={() => removeRefBand(i)}
									>{i18n.t('modal.chart.refLines.remove')}
								</Button>
							</div>
						{/each}
					</div>
					<Button variant="outline" size="sm" class="ref__add" onclick={addRefBand}
						>{i18n.t('modal.chart.refBands.add')}</Button
					>
				</div>
			{/if}
			...
			{#if showCondFormat}
				<div class="cond">
					<span class="text-sm text-fg-muted"
						>{i18n.t('modal.chart.cond.title', 'Conditional formatting')}</span
					>
					<p class="m-0 text-xs text-fg-subtle">
						{i18n.t(
							'modal.chart.cond.hint',
							'Recolour bars by value — the first matching rule per bar wins.'
						)}
					</p>
								...
								<div class="flex items-center gap-2">
									<select
										...
										onchange={(e) =>
											changeCondOp(
												mi,
												ri,
												(e.currentTarget as HTMLSelectElement).value as ChartCondOp
											)}
									...
									{#if rule.op === 'between'}
										<input
											...
											oninput={(e) =>
												updateCondRule(mi, ri, { value: [numVal(e), tupleAt(rule.value, 1)] })}
										...
										<input
											...
											oninput={(e) =>
												updateCondRule(mi, ri, { value: [tupleAt(rule.value, 0), numVal(e)] })}
								...
								{#if (condBand(mi)?.rules.length ?? 0) > 0}
									<label class="inline-flex items-center gap-2 text-xs text-fg-muted">
										...
										<input
											...
											onchange={(e) =>
												setCondFallback(mi, (e.currentTarget as HTMLInputElement).value)}

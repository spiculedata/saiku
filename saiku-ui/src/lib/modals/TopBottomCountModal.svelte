<script lang="ts">
	...
	let { axis, variant, measures, initialCount, initialMeasure, open, onSave, onCancel }: Props =
		$props();
	...
	const valid = $derived(
		mode === 'simple' ? !!selected && count >= 1 : mdxBuffer.trim().length > 0
	);

	const titleKey = $derived(
		variant === 'top' ? 'modal.filter.topCount' : 'modal.filter.bottomCount'
	);
</script>
...
>
	...
	{#if mode === 'simple'}
		<label class="field">
			...
			<input class="field__input" type="number" min="1" step="1" bind:value={count} />
		</label>
		<label class="field">

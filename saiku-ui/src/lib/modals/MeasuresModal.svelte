<script lang="ts">
	...
	const filtered = $derived(
		available.filter(
			(m) => !search || (m.caption || m.name).toLowerCase().includes(search.toLowerCase())
		)
	);
		...
		{#each filtered as m}
			<li>
				<label>
					<input
						type="checkbox"
						checked={picks.has(m.uniqueName)}
					...
					{#if m.visible === false}<span class="badge badge--hidden"
							>{i18n.t('modal.measures.hiddenBadge')}</span
						>{/if}
				</label>
			</li>
		{/each}

<script lang="ts">
	...
	const parallelLevel = $derived.by(() => {
		...
		const match = hierarchyLevels.find((l) => grainFromLevelType(l.levelType) === targetGrain);
...
<Modal
	title={`${i18n.t('modal.dateFilter.title')} — ${hierarchyCaption}`}
	{open}
	size="md"
	onClose={onCancel}
>
	<div class="tabs" role="tablist">
		<button
			...
			onclick={() => (tab = 'relative')}>{i18n.t('modal.dateFilter.tab.relative')}</button
		>
		<button
			type="button"
			role="tab"
			class:active={tab === 'absolute'}
			onclick={() => (tab = 'absolute')}>{i18n.t('modal.dateFilter.tab.absolute')}</button
		>
		<button
			type="button"
			role="tab"
			class:active={tab === 'compare'}
			onclick={() => (tab = 'compare')}>{i18n.t('modal.dateFilter.tab.compare')}</button
		>
	</div>
	...
	{#if tab === 'relative'}
		<label class="field">
			...
			{#if currentGrain && presetsAllowed.length !== PRESETS.length}
				<span class="hint"
					>{i18n.t('modal.dateFilter.grainHint').replace('{grain}', currentGrain)}</span
				>
			{/if}
		</label>
		{#if takesN}

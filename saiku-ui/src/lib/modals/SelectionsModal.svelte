<script lang="ts">
	...
	function parentPath(uniqueName: string): string {
		const segs = uniqueName.match(/\[([^\]]+)\]/g)?.map((s) => s.slice(1, -1)) ?? [];
...
>
	...
	<div class="row">
		...
		<label class="field">
			...
			<select
				...
				onchange={(e) => setType((e.currentTarget as HTMLSelectElement).value as SelectionType)}
			>
	...
	<div class="bar">
		...
		<span class="ml-auto text-sm text-fg-muted">
			{activeSlot?.selected.size ?? 0}
			{i18n.t('modal.selections.selected')}
		</span>
	</div>
				...
				<li>
					<label>
						...
						<span class="flex flex-1 flex-col">
							...
							{#if parents}
								<span class="text-xs text-fg-subtle">{parents}</span>
							{/if}
						</span>
						{#if m.description}
							<span class="text-xs text-fg-subtle">{m.description}</span>
						{/if}
					</label>
				</li>
			{/each}

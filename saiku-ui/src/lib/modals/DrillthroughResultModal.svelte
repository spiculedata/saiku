...
<Modal title={i18n.t('modal.drillthroughResult.title')} {open} size="xl" {onClose}>
	{#if !result}
		<p class="p-3 text-fg-muted">{i18n.t('cubes.loading')}</p>
	{:else if result.error}
		<p class="callout callout--danger">{result.error}</p>
	{:else if !result.cellset || result.cellset.length === 0}
		<p class="p-3 text-fg-muted">{i18n.t('modal.drillthroughResult.noRows')}</p>
	{:else}
		<div class="scroll">
			<table class="dt">
				<tbody>
		...
		<p class="mt-2 text-xs text-fg-subtle">
			{(result.cellset.length - 1).toLocaleString()}
			{i18n.t('units.rows')} · {result.runtime ?? 0}
			{i18n.t('units.ms')}
		</p>
	{/if}
	{#snippet footer()}

...
function readMetadata(schemaMetadata: Map<string, string> | undefined): CellsetMetadata {
	...
	if (typeof parsed.rowHeaderColCount !== 'number' || !Array.isArray(parsed.columnHeaderRows)) {
...
function readDrillthroughMetadata(
	...
	if (!raw) {
		throw new Error(
			`Arrow drillthrough stream missing ${DRILLTHROUGH_METADATA_KEY} metadata entry`
		);
	}

...
export async function executeOssieQuery(
	...
	try {
		...
		throw new Error(
			`executeOssieQuery -> ${res.status}: non-JSON response (${(e as Error).message})`
		);
	}
}
...
export async function runSharedTileQuery(
	...
	try {
		...
		throw new Error(
			`runSharedTileQuery -> ${res.status}: non-JSON response (${(e as Error).message})`
		);
	}
}
...
export async function detectAnomalies(
	...
	try {
		...
		throw new Error(
			`detectAnomalies -> ${res.status}: non-JSON response (${(e as Error).message})`
		);
	}
}
...
export async function executeSavedQuery(
	...
	try {
		...
		throw new Error(
			`executeSavedQuery -> ${res.status}: non-JSON response (${(e as Error).message})`
		);
	}
}

			...
			catalogs: [
				{
					...
					schemas: [
						{
							name: 'sch',
							cubes: [cube('Sales', 'conn-a'), cube('Inventory', 'conn-a')]
						}
					]
				}
			]
...
describe('pickStarterLevelDrop()', () => {
	...
	it('returns a LevelDrop with all 7 chip fields populated', () => {
		const dims: SaikuDimension[] = [dimension('Region', [hierarchy('Region', [level('Region')])])];
		const drop = pickStarterLevelDrop(dims);
		expect(drop).toEqual({
...
describe('pickStarterMeasureAndLevel()', () => {
	...
	it('returns null when dimensions are empty', () => {
		expect(pickStarterMeasureAndLevel([measure('Revenue')], [])).toBeNull();
	});

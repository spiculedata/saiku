...
export function createSchemaGenStore(client: SchemaGenClient, pollMs = 500): SchemaGenStore {
	...
	async function refreshViews(sessionId: string): Promise<void> {
		try {
			const [d, sug] = await Promise.all([client.draft(sessionId), client.suggestions(sessionId)]);
			s.draft = d;
			s.suggestions = sug;
		} catch (err) {

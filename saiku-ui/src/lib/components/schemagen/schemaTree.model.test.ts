...
describe("buildTree", () => {
  ...
  it("produces the expected shape for a populated draft", () => {
    ...
    expect(hier.path).toBe(
      "cubes/Orders/dimensions/Customer/hierarchies/Geography",
    );
...
describe("resolveDraftNode", () => {
  ...
  it("resolves cube, dim, hierarchy, level, measure, and shared dim paths", () => {
    ...
    expect(
      resolveDraftNode(draft, "cubes/Orders/dimensions/Customer")?.kind,
    ).toBe("dimension");
    expect(
      resolveDraftNode(
        draft,
    ...
    expect(resolveDraftNode(draft, "cubes/Orders/measures/Revenue")?.kind).toBe(
      "measure",
    );
    expect(resolveDraftNode(draft, "sharedDimensions/Time")?.kind).toBe(
      "sharedDim",
    );
    expect(
      resolveDraftNode(draft, "sharedDimensions/Time/hierarchies/Calendar")
        ?.kind,
    ).toBe("hierarchy");
  });

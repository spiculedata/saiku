...
<div class="prefs-menu">
	...
	{#if open}
		<div
			class="prefs-menu__panel {placement === 'down' ? 'prefs-menu__panel--down' : ''}"
			role="menu"
		>
			...
			<div class="flex items-center justify-between gap-3">
				...
				<Button
					variant="outline"
					size="sm"
					class={theme.colorBlindSafe ? 'btn--active' : ''}
					role="switch"
				...
				>
					...
					<span
						>{theme.colorBlindSafe
							? i18n.t('topbar.contrast.on')
							: i18n.t('topbar.contrast.off')}</span
					>
				</Button>
			</div>
			<div class="flex items-center justify-between gap-3">

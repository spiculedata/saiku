...
describe("formatNumber — inert / null handling", () => {
  ...
  test("an all-empty format is treated as inert (raw value)", () => {
    ...
    expect(formatNumber(1234.5, { prefix: "", suffix: "", decimals: null, thousands: false, abbreviate: false })).toBe(
      "1234.5",
    );
  });
...
describe("formatNumber — prefix / suffix", () => {
  test("prefix and suffix wrap the formatted core", () => {
    expect(formatNumber(1234.5, { prefix: "£", decimals: 2, thousands: true }).startsWith("£")).toBe(true);

	...
	return {
		...
		data: lines.map((l) => {
			...
			const point: Record<string, unknown> =
				l.axis === 'x' ? { xAxis: l.value } : { yAxis: l.value };
...
export function brushOption(chartType: string): Record<string, unknown> | null {
	...
	return {
		...
		brushStyle: {
			borderWidth: 1,
			color: 'rgba(120,140,210,0.18)',
			borderColor: 'rgba(120,140,210,0.65)'
		}
	};
}
	...
	return (params: AxisTipParam | AxisTipParam[]): string => {
		...
		const rows = list
			.map((p) => {
				...
				const spark =
					withSparkline && colSeries.has(name)
...
export function buildChartOption(
	...
	const aspect =
		Number.isFinite(geom.aspect) && (geom.aspect ?? 0) > 0 ? (geom.aspect as number) : 1;
	...
	const catLabel = (rotate = 0, axisCategoryCount = cols.length || 1) => {
		...
		const w = compact ? 100 : deriveAxisLabelWidth(geom.chartWidth ?? 0, axisCategoryCount);
	...
	const catNameCfg = (rotated: boolean): Record<string, unknown> =>
		categoryName
			? {
					name: categoryName,
					nameLocation: 'middle',
					nameGap: rotated ? (compact ? 52 : 68) : compact ? 26 : 32
				}
			: { name: undefined };
	...
	if (t === 'pie' || t === 'donut' || t === 'treemap' || t === 'sunburst') {
		...
		const showSliceLabels = (collapseToSingle ? cols.length : rows.length) <= MAX_LABELLED_SLICES;
		...
		return {
			...
			series: cells.map((cell, m) => ({
				...
				data: (collapsedData ?? rows.map((name, i) => ({ name, value: matrix[i][m] ?? 0 }))).filter(
					(d) => d.value > 0
				)
			}))
		};

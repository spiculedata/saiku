...
describe("matchRule", () => {
  ...
  test("malformed value shapes never match", () => {
    expect(matchRule(5, { op: "between", value: 5 as unknown as [number, number], color: "g" })).toBe(false);
...
describe("colorForValue", () => {
  ...
  test("no fallback → undefined (keep palette) when nothing matches", () => {
    const noFallback: ChartConditionalFormat = { measureIndex: 0, rules: [{ op: "gt", value: 1000, color: "green" }] };
  ...
  test("null / non-finite / empty-rules → undefined", () => {
    ...
    expect(colorForValue(1500, { measureIndex: 0, rules: [], fallbackColor: "grey" })).toBeUndefined();

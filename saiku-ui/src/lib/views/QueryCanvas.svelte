<script lang="ts">
	...
	function onMenuPick(id: string) {
		...
		if (m.kind === 'axis' && m.axis) {
			...
			if (id === 'filter-order') {
				type = 'Order';
				placeholder = '[Measures].[Unit Sales]';
			} else if (id === 'filter-filter') {
				type = 'Filter';
				placeholder = '[Measures].[Unit Sales] > 100000';
			} else if (id === 'filter-top') {
				type = 'TopCount';
				placeholder = '10, [Measures].[Unit Sales]';
			} else if (id === 'filter-bot') {
				type = 'BottomCount';
				placeholder = '10, [Measures].[Unit Sales]';
			} else if (id === 'filter-limit') {
				type = 'Limit';
				placeholder = '10';
			}
	...
	function onCustomFilterApply(op: string, value: string, value2?: string) {
		...
		if (op === 'BETWEEN') expr = `${m.uniqueName} >= ${value} AND ${m.uniqueName} <= ${value2}`;
			...
			expr = `NOT (${m.uniqueName} >= ${value} AND ${m.uniqueName} <= ${value2})`;
	...
	function onFormatPctApply(base: 'ROWS' | 'COLUMNS' | 'GRAND_TOTAL', _scope: 'all' | 'selected') {
		...
		if (base === 'ROWS') {
			const p = primaryRowsHier();
			if (!p) {
				toasts.warning(i18n.t('toast.noRowsHier'), i18n.t('toast.noRowsHier.pct'));
				return;
			}
			...
			if (!c) {
				toasts.warning(i18n.t('toast.noColsHier'), i18n.t('toast.noColsHier.pct'));
				return;
			}
		...
		next.push({
			name: calcName,
			formula,
			properties: { FORMAT_STRING: '0.00%', SOLVE_ORDER: '200' }
		});
		query.current.queryModel.calculatedMeasures = next;
		query.addMeasure({
			name: calcName,
			uniqueName: `[Measures].[${calcName}]`,
			caption: calcName,
			type: 'CALCULATED'
		});
	...
	function onGrowthApply(basis: string, ref?: string) {
		...
		const p = primaryRowsHier();
		if (!p) {
			toasts.warning(i18n.t('toast.noRowsHier'), i18n.t('toast.noRowsHier.growth'));
			return;
		}
		...
		if (basis === 'previous') prev = `(${m.uniqueName}, ${p.hier}.CurrentMember.PrevMember)`;
			...
			toasts.warning(i18n.t('toast.refMissing'), i18n.t('toast.refMissing.body'));
			return;
		}
		...
		next.push({
			name: calcName,
			formula,
			properties: { FORMAT_STRING: '0.00%', SOLVE_ORDER: '300' }
		});
		query.current.queryModel.calculatedMeasures = next;
		query.addMeasure({
			name: calcName,
	...
	function onAxisFilterSave(expression: string, sort?: string) {
		...
		if (!base) {
			toasts.warning(
				`${t.axis} ${i18n.t('toast.axisEmpty')}`,
				i18n.t('toast.axisEmpty.body').replace('{axis}', t.axis)
			);
			return;
		}
	...
	$effect(() => {
		...
		if (
			selection.cube &&
			(!query.current || query.current.cube.uniqueName !== selection.cube.uniqueName)
		) {
	...
	function clearDragOver() {
		dragOverAxis = null;
		dragOverChipKey = null;
		dragOverChipAfter = false;
		dragSourceAxis = null;
	}
	...
	function onChipDrop(
		e: DragEvent,
		targetAxis: ZoneId,
		target: { kind: 'hierarchy'; name: string } | { kind: 'measure'; uniqueName: string }
	) {
		const chipPayload = e.dataTransfer?.getData('application/x-saiku-chip');
		...
		if (
			payload.kind === 'hierarchy' &&
			target.kind === 'hierarchy' &&
			payload.axis === targetAxis
		) {
	...
	async function openSelections(axis: AxisLocation, hier: ThinHierarchy, preferredLevel?: string) {
		if (!selection.cube || !session.current) return;
		const pickedLevels = sortLevelsBySchemaOrder(hier);
		if (pickedLevels.length === 0) return;
		const initialLevelName =
			preferredLevel && pickedLevels.includes(preferredLevel) ? preferredLevel : pickedLevels[0];
		...
		const initialPerLevel: Record<string, { selected: string[]; type: 'INCLUSION' | 'EXCLUSION' }> =
			{};
	...
	async function loadSelectionsLevelMembers(
		...
		try {
			...
			try {
				return await listRootMembers(session.current.username, selection.cube, hier.name);
			} catch (err2) {
				toasts.danger(
	...
	let cubeMetadata = $state<{
		dimensions: import('$lib/api/discover').SaikuDimension[];
		measures: import('$lib/api/discover').SaikuMeasure[];
	} | null>(null);
	$effect(() => {
		...
		datasources
			.metadata(session.current.username, cube)
			.then((md) => (cubeMetadata = md))
			.catch(() => {});
	});
	...
	async function onSelectionsSaveSingle(
		...
		query.setLevelSelection(selectionsTarget.hierarchyName, levelName, uniqueNames, type);
		selectionsOpen = false;
		toasts.success(
			i18n.t('toast.saved'),
			i18n.t('toast.selectionsApplied').replace('{n}', String(uniqueNames.length))
		);
...
<div class="canvas">
	{#if !selection.cube}
		<div class="canvas__empty">
			...
			<p class="m-0 text-sm text-fg-subtle">{i18n.t('canvas.pickPrompt')}</p>
		</div>
	{:else}
		<div
			bind:this={bodyEl}
		...
		>
           ...
           so the MDX emission is unchanged. -->
					<div
						class={dragOverAxis === 'MEASURES'
							? 'dropzone dropzone--measures is-dragover'
							: 'dropzone dropzone--measures'}
						role="region"
					...
					>
						<header>
							...
							<button
								type="button"
								class="dropzone__menu"
								title={i18n.t('canvas.menu.position')}
								aria-label={i18n.t('canvas.menu.position')}
								onclick={openMeasuresMenu}
							>
						...
						<div class="chips">
							{#each query.current?.queryModel?.details.measures ?? [] as m}
								...
								<span
									class={dragOverChipKey === chipKey('MEASURES', 'measure', m.uniqueName)
										? 'chip chip--measure is-drop-before'
										: 'chip chip--measure'}
									draggable="true"
									...
									ondrop={(e) =>
										onChipDrop(e, 'MEASURES', { kind: 'measure', uniqueName: m.uniqueName })}
								...
								>
									...
									<button
										...
										onclick={() => removeMeasure(m.uniqueName)}>×</button
									>
								</span>
							{/each}
						...
						>
							<header>
								...
								<button
									type="button"
									class="dropzone__menu"
									title={i18n.t('canvas.axisOptions')}
									aria-label={i18n.t('canvas.axisOptions')}
									onclick={(e) => openAxisMenu(e, axis)}
								>
									<MoreHorizontal size={14} />
								</button>
							</header>
                     ...
                     selections modal (member picker). -->
										<div class="chip-group__head">
											...
											<button
												...
												onclick={() => removeHier(h.name)}>×</button
											>
										</div>
					...
					{/each}
					<div
						class={dragOverAxis === 'FILTER'
							? 'dropzone dropzone--filter is-dragover'
							: 'dropzone dropzone--filter'}
						role="region"
					...
					>
						<header>
							...
							<button
								...
								title={i18n.t('canvas.axisOptions')}
								aria-label={i18n.t('canvas.axisOptions')}
								onclick={(e) => openAxisMenu(e, 'FILTER')}
							>
								<MoreHorizontal size={14} />
							</button>
						</header>
						<div class="chips">
							{#each query.current?.queryModel?.axes.FILTER.hierarchies ?? [] as h}
								<!-- svelte-ignore a11y_no_static_element_interactions — drag/context-menu are mouse affordances; the inner buttons (label, close) handle keyboard accessibility. -->
								<span
									class={dragOverChipKey === chipKey('FILTER', 'hierarchy', h.name)
										? 'chip chip--level is-drop-before'
										: 'chip chip--level'}
									draggable="true"
			...
			<div class="flex min-h-0 min-w-0 flex-col gap-2 overflow-hidden">
				{#if query.current?.type === 'MDX'}
					<div class="canvas__mdx-banner" role="status" title={i18n.t('canvas.mdxBannerText')}>
						...
						<span class="min-w-0 overflow-hidden text-ellipsis"
							>{i18n.t('canvas.mdxBannerShort')}</span
						>
					</div>
				{/if}
				<div class="view-toggle" role="tablist" aria-label={i18n.t('canvas.resultView')}>
					<button
						type="button"
						role="tab"
						class:active={query.viewMode === 'grid'}
						onclick={() => (query.viewMode = 'grid')}
					>
					...
					<button
						type="button"
						role="tab"
						class:active={query.viewMode === 'chart'}
						onclick={() => (query.viewMode = 'chart')}
					>
					...
					<div class="view-more">
						...
						>
							<span
								>{isMoreMode(query.viewMode)
									? viewModeLabel(query.viewMode)
									: i18n.t('canvas.view.more')}</span
							>
							<ChevronDown size={14} />
						</button>
						{#if moreViewOpen}
					...
					{#if query.viewMode === 'chart'}
						...
						<button
							...
							title={i18n.t('modal.chart.title')}
							aria-label={i18n.t('a11y.editChartOptions')}
							onclick={() => (modals.chartEditor = true)}
						>
							<Settings size={18} />
						</button>
					{/if}
				...
				{#if query.running}
					<div class="run-progress" role="status" aria-live="polite">
						...
						<span
							>{i18n.t('canvas.running.short')} ({(query.runningElapsedMs / 1000).toFixed(
								1
							)}s)</span
						>
						<button
							...
							title={i18n.t('canvas.cancelQuery')}
						>
							<XCircle size={14} />
...
{#if selectionsTarget}
	...
	<SelectionsModal
		...
		open={selectionsOpen}
		loadMembers={(lvl) => (hier ? loadSelectionsLevelMembers(hier, lvl) : Promise.resolve([]))}
...
<ChartEditorModal
	...
	onSave={(next) => {
		query.chartOptions = next;
		modals.chartEditor = false;
	}}
...
<GrowthModal open={!!modals.growth} onApply={onGrowthApply} onCancel={() => closeModal('growth')} />
...
<style>
	...
	.dropzone:hover .dropzone__menu {
		opacity: 1;
	}
	.dropzone__menu:hover {
	...
	.view-more__menu button:hover {
		background: hsl(var(--bg-subtle));
	}
	.view-more__menu button.active {
		background: hsl(var(--primary));
		color: hsl(var(--primary-foreground));
	}
	.chart-pick select {

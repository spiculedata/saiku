<script lang="ts">
	 ...
	 */
	let runnable = $derived.by(
		(): { ok: true } | { ok: false; reasonKey: string; hintKey: string } => {
			const q = query.current?.queryModel;
			if (!q)
				return {
					ok: false,
					reasonKey: 'toolbar.run.disabled.noQuery',
					hintKey: 'toolbar.run.disabled.noQuery.hint'
				};
			if (q.details.measures.length === 0) {
				return {
					ok: false,
					reasonKey: 'toolbar.run.disabled.noMeasure',
					hintKey: 'toolbar.run.disabled.noMeasure.hint'
				};
			}
			const hasHierarchy =
				q.axes.COLUMNS.hierarchies.length > 0 || q.axes.ROWS.hierarchies.length > 0;
			if (!hasHierarchy) {
				return {
					ok: false,
					reasonKey: 'toolbar.run.disabled.noHierarchy',
					hintKey: 'toolbar.run.disabled.noHierarchy.hint'
				};
			}
			return { ok: true };
		}
	...
	async function onDrillAcross(target: SaikuCube) {
		...
		toasts.info(
			i18n.t('toast.drillAcross'),
			i18n.t('toast.drillAcross.body').replace('{cube}', target.caption || target.name)
		);
	}
...
<div class="toolbar" role="toolbar" aria-label="Workspace toolbar">
	<div class="relative flex items-center gap-0.5" role="group" aria-label="File">
		...
		<Tooltip
			text={query.dirty ? `${i18n.t('toolbar.save')} (unsaved changes)` : i18n.t('toolbar.save')}
		>
			<button class="tb-btn relative" aria-label={i18n.t('toolbar.save')} onclick={onSave}>
				<Save size={18} />
				{#if query.dirty}
	...
	<div class="relative flex items-center gap-0.5" role="group" aria-label="Undo / redo">
	...
	<div class="toolbar__sep"></div>
	<div class="relative flex items-center gap-0.5" role="group" aria-label="Query">
		<div class="split-btn">
			<button
	...
	<div
		...
		role="group"
		aria-label={i18n.t('workspace.aiQuery.title')}
	>
		<button
			class="tb-btn tb-btn--ai"
			disabled={!aiConfigured}
			title={aiConfigured
				? i18n.t('workspace.aiQuery.open')
				: i18n.t('workspace.aiQuery.disabled', "AI isn't configured on this server")}
	...
	<div class="toolbar__sep"></div>
	<div class="relative flex items-center gap-0.5" role="group" aria-label="Tools">
		<button
			class="tb-btn"
			onclick={(e) => {
				e.stopPropagation();
		...
		>
			<Wrench size={18} /><span class="text-sm">{i18n.t('toolbar.tools')}</span><ChevronDown
				size={14}
			/>
		</button>
		{#if toolsMenuOpen}
	...
	<div class="relative flex items-center gap-0.5" role="group" aria-label="Export">
		<button
			class="tb-btn"
			onclick={(e) => {
				e.stopPropagation();
		...
		>
			<Download size={18} /><span class="text-sm">{i18n.t('toolbar.export')}</span><ChevronDown
				size={14}
			/>
		</button>
		{#if exportMenuOpen}
			<div class="toolbar__dropdown right-0 left-auto">
				...
				<button
					...
					title={mailHealth.configured
						? i18n.t('toolbar.emailMeThis.tooltip', 'Email this report')
						: i18n.t('toolbar.emailMeThis.disabled', "Email isn't configured on this server")}
...
<MDXModal mdx={mdxBuffer} open={mdxOpen} onRun={onRunMdx} onCancel={() => (mdxOpen = false)} />

<SavedQueriesModal open={openOpen} {onOpenQuery} onClose={() => (openOpen = false)} />
...
<style>
	...
	.toolbar__item:hover:not(:disabled) {
	...
	.tb-btn--ai {
		background: linear-gradient(
			135deg,
			hsl(var(--primary)) 0%,
			color-mix(in srgb, hsl(var(--primary)) 60%, #8b5cf6) 100%
		);
		color: hsl(var(--bg));
		border-color: hsl(var(--primary));
	}

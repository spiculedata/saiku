...
<div class="ossie-tree">
  ...
  {:else if ossieQuery.model}
    <section class="ossie-tree__section">
      ...
      <p class="ossie-tree__hint">Metrics reference the fact dataset. Cross-dataset joins are auto-injected.</p>
    </section>
      ...
      <section class="ossie-tree__section">
        ...
        <ul class="ossie-tree__list ossie-tree__list--dim">
          {#each ossieQuery.model.relationships as r}
            <li class="ossie-tree__relation" title={`${r.fromColumns.join(",")} → ${r.toColumns.join(",")}`}>
              <span>{r.from}</span>

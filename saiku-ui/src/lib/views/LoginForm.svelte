	...
	{:else}
		<form class="login" onsubmit={onSubmit}>
			...
			{#if showDemoPanel}
				<div class="demo-creds">
					...
					<div class="flex flex-col gap-0.5">
						...
						<span>
							Sign in with <code>admin</code> / <code>admin</code>, or use the shortcut below. Data
							resets nightly.
						</span>
					</div>
				</div>
			...
			{#if showDemoPanel}
				<Button
					variant="outline"
					class="login__demo-button w-full"
					onclick={loginAsDemo}
					disabled={busy}
				>
		...
		{#if showDemoPanel}
			<aside class="w-full rounded-lg border border-border bg-bg p-4">
				<ApiAccessAdmin />
			</aside>
		{/if}
	{/if}

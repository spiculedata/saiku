...
<div class="insp-section">
  ...
  {#if nav.position !== "top"}
    ...
    <label class="insp-row insp-toggle"><span>Full height (header beside rail)</span>
      <input type="checkbox" checked={nav.railFullHeight ?? false}
        onchange={(e) => appDoc.updateNav({ railFullHeight: (e.currentTarget as HTMLInputElement).checked })} />
    </label>
    <p class="insp-hint">
      Off: the header spans the top and the rail starts below it. On: the rail is
      the outermost chrome, running the full height with the header to its right.
    </p>
  {/if}
</div>
...
{#if nav.position !== "top"}
  <div class="insp-section">
    ...
    <div class="insp-row"><span>User disc</span>
      <div class="insp-seg">
        <button type="button" class:is-active={source === "user"}
          onclick={() => setFooter({ avatarSource: "user" })}>Signed-in user</button>
        <button type="button" class:is-active={source === "fixed"}
          onclick={() => setFooter({ avatarSource: "fixed" })}>Fixed</button>
        <button type="button" class:is-active={source === "off"}
          onclick={() => setFooter({ avatarSource: undefined, avatar: undefined })}>Off</button>
      </div>
    </div>
    {#if source === "fixed"}
      <label class="insp-row"><span>Initials</span>
    ...
    <p class="insp-hint">
      {#if source === "user"}
        Initials come from whoever is signed in — the disc identifies the viewer,
        not the author.
      {:else if source === "fixed"}
        The same initials for every viewer. Useful for a mock-up or a shared
        kiosk; not for a real user avatar.
      {:else}

...
<div class="insp-section">
	<label class="insp-row insp-toggle"
		><span>Show assistant panel</span>
		<input
			type="checkbox"
			checked={a.enabled}
...
{#if a.enabled}
	<div class="insp-section">
		...
		<label class="insp-row"
			...
			<input
				class="insp-input"
				placeholder={appDoc.current?.name ?? 'this app'}
				value={a.title ?? ''}
				oninput={(e) => set({ title: opt(val(e)) })}
			/>
		</label>
		<label class="insp-row"
			><span>Persona</span>
			<input
				...
				value={a.persona ?? ''}
				oninput={(e) => set({ persona: opt(val(e)) })}
			/>
		</label>
		<label class="insp-row"
			...
			<input
				class="insp-input"
				placeholder="scoped to your stores"
				value={a.scope ?? ''}
				oninput={(e) => set({ scope: opt(val(e)) })}
			/>
		</label>
		<label class="insp-row"
			><span>Icon</span>
			<div class="insp-seg">
				<button
					type="button"
					class:is-active={(a.icon ?? 'sparkles') === 'sparkles'}
					onclick={() => set({ icon: 'sparkles' })}>Sparkles</button
				>
				<button
					type="button"
					class:is-active={a.icon === 'crosshair'}
					onclick={() => set({ icon: 'crosshair' })}>Crosshair</button
				>
			</div>
		</label>
	</div>
	...
	<div class="insp-section">
		...
		<textarea
			...
			value={a.greeting ?? ''}
			oninput={(e) => set({ greeting: opt((e.currentTarget as HTMLTextAreaElement).value) })}
		></textarea>
	</div>
	...
	<div class="insp-section">
			...
			{#each prompts as p, i (i)}
				<div class="insp-list-item">
					<input
						class="insp-input"
						value={p}
						oninput={(e) => set({ suggestedPrompts: editList(prompts, i, val(e)) })}
					/>
					<button
						type="button"
						class="insp-iconbtn"
						aria-label="Remove"
						onclick={() => set({ suggestedPrompts: prompts.filter((_, j) => j !== i) })}
		...
		<button
			type="button"
			class="insp-addbtn"
			onclick={() => set({ suggestedPrompts: [...prompts, ''] })}
		>
	...
	<div class="insp-section">
			...
			{#each skills as s, i (i)}
				<div class="insp-list-item">
					<input
						class="insp-input"
						value={s}
						oninput={(e) => set({ skillPrompts: editList(skills, i, val(e)) })}
					/>
					<button
						type="button"
						class="insp-iconbtn"
						aria-label="Remove"
						onclick={() => set({ skillPrompts: skills.filter((_, j) => j !== i) })}
						><Trash2 size={14} /></button
					>
				</div>
			{/each}
		...
		<button
			type="button"
			class="insp-addbtn"
			onclick={() => set({ skillPrompts: [...skills, ''] })}
		>
	...
	<div class="insp-section">
		...
		<label class="insp-row"
			...
			<input
				class="insp-input"
				placeholder="↵ to send · ⇧↵ new line"
				value={a.footerHint ?? ''}
				oninput={(e) => set({ footerHint: opt(val(e)) })}
			/>
		</label>
		<label class="insp-row"
			...
			<input
				class="insp-input"
				placeholder="powered by Saiku"
				value={a.poweredBy ?? ''}
				oninput={(e) => set({ poweredBy: opt(val(e)) })}
			/>
		</label>
	</div>
{/if}

...
<div class="insp-section">
  ...
  <label class="insp-row"><span>Headings</span>
    <select class="insp-select" value={tok.fontDisplay}
  ...
  <label class="insp-row"><span>Body</span>
    <select class="insp-select" value={tok.fontBody}
...
<div class="insp-section">
  ...
  <div class="insp-row"><span>Corners</span>
    <div class="insp-seg">
      {#each RADII as r (r)}<button type="button" class:is-active={tok.radius === r} onclick={() => setToken("radius", r)}>{r}</button>{/each}
    </div>
  </div>
  <div class="insp-row"><span>Shadow</span>
    <div class="insp-seg">
      {#each SHADOWS as sh (sh)}<button type="button" class:is-active={tok.shadow === sh} onclick={() => setToken("shadow", sh)}>{sh}</button>{/each}
    </div>
  </div>
  <div class="insp-row"><span>Density</span>
    <div class="insp-seg">
...
<div class="insp-section">
  <button type="button" class="disclosure" aria-expanded={advancedOpen} onclick={() => (advancedOpen = !advancedOpen)}>
    {advancedOpen ? "▾" : "▸"} Advanced — custom CSS
  </button>
  {#if advancedOpen}
    <p class="insp-hint">Escape hatch for the long tail. Sanitised + scoped to this app — you shouldn't need it for standard branding.</p>
    <textarea class="insp-textarea" rows="8" spellcheck="false" placeholder="/* scoped custom CSS */"
      value={theme.customCss ?? ""}
      oninput={(e) => appDoc.setTheme({ customCss: (e.currentTarget as HTMLTextAreaElement).value })}
    ></textarea>
  {/if}
</div>

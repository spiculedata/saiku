<script lang="ts">
	...
	async function send(text: string): Promise<void> {
		...
		if (!cube) {
			messages = [
				...messages,
				{ role: 'assistant', text: 'No cube is bound to this app yet.', kind: 'error' }
			];
			return;
		}
...
<aside class="assistant" aria-label="Assistant">
	...
	<div class="assistant__persona">
		Persona <span class="assistant__pill">{persona}</span>{#if scope}<span class="assistant__scope">
				· {scope}</span
			>{/if}
	</div>
	{#if scopeNote}
		...
		{#each messages as m}
			<div class="assistant__msg assistant__msg--{m.role}" class:is-error={m.kind === 'error'}>
             ...
             is preserved for apps that never set one. -->
				{#if m.kind === 'greeting'}<div class="assistant__eyebrow">
						{persona.toUpperCase()}
					</div>{/if}
				<p>{m.text}</p>
			</div>
		{/each}
		{#if busy}<div class="assistant__msg assistant__msg--assistant assistant__typing">
	...
	{#if prompts.length > 0 || skillPrompts.length > 0}
		...
		<div class="assistant__prompts">
			{#each prompts as p}
				<button type="button" class="assistant__chip" disabled={busy} onclick={() => void send(p)}
					>{p}</button
				>
			{/each}

...
import { importArgsFromDashboard, pathStem, slugify, composeAppPath } from './appImport';
...
describe('importArgsFromDashboard', () => {
	...
	it('falls back to the path stem when the dashboard has no name', () => {
		const args = importArgsFromDashboard({ name: '', layout }, 'homes/admin/q4-sales.saikudash');
...
describe('composeAppPath', () => {
	it('joins folder and slugified name with the .saikuapp extension', () => {
		expect(composeAppPath('homes/admin', 'Sales App')).toBe('homes/admin/sales-app.saikuapp');
	});
	...
	it('strips stray slashes on the folder', () => {
		expect(composeAppPath('/homes/admin/', 'Sales')).toBe('homes/admin/sales.saikuapp');
	});
});

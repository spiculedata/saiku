...
import { withGreeting, DEFAULT_GREETING, type AssistantMessage } from './appAssistant';
...
describe('withGreeting', () => {
	test('seeds the greeting into an empty list', () => {
		...
		expect(out).toEqual([{ role: 'assistant', text: 'Ask me about scripts.', kind: 'greeting' }]);
	});
...
describe('assistant composer placeholder is app-derived (saiku#1791)', () => {
	const src = readFileSync(resolve(process.cwd(), 'src/lib/views/app/AppAssistant.svelte'), 'utf8');
	...
	test('carries no sample-dataset vocabulary', () => {
		...
		for (const word of ['sales', 'products', 'FoodMart', 'stores', 'warehouse']) {
			expect(placeholder.toLowerCase(), `composer placeholder still names "${word}"`).not.toContain(
				word.toLowerCase()
			);
		}
	});

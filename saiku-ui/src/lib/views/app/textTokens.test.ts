...
import { isoWeek, renderTokens, tokenHelp, FILTER_TOKEN_PLACEHOLDER } from './textTokens';
...
describe('tokenHelp', () => {
	...
	it("names the app's own dimension in the filter chip", () => {
		const chip = tokenHelp('Geography').find((t) => t.token.startsWith('{filter:'));
	...
	it("falls back to a neutral placeholder, never a sample dataset's dimension", () => {
		...
		expect(tokenHelp('   ').find((t) => t.token.startsWith('{filter:'))?.token).toBe(
			`{filter:${FILTER_TOKEN_PLACEHOLDER}}`
		);
	});
});

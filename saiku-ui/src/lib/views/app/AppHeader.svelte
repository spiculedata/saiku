...
<header class="saiku-app__header">
	...
	>
		...
		{#if header.wordmarkAccent}
			<span class="saiku-app__name"
				>{wordmark[0]}<span class="saiku-app__name-accent">{wordmark[1]}</span>{wordmark[2]}</span
			>
		{:else}
	...
	<div class="saiku-app__controls">
		{#if pill}
           ...
           free (the same trick the reference mock-up uses). -->
			<div
				class="saiku-app__ctxpill"
				class:saiku-app__ctxpill--live={pillSelectable}
				title={pillValue}
			>

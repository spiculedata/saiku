	...
	>
		...
		>
			...
			{#if aiDashboardBuild.request}
				<div class="max-w-full text-sm break-words text-fg-muted">{aiDashboardBuild.request}</div>
			{/if}
			<div class="text-sm text-fg-subtle tabular-nums">{mmss}</div>
			<div class="text-xs text-fg-subtle">
				{i18n.t(
					'workspace.aiDashboard.buildingHint',
					'This can take a minute or two on a local model.'
				)}
			</div>

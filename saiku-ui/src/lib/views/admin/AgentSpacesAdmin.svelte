<script lang="ts">
	...
	const cubeKey = (c: AgentSpaceCubeRef | AiCubeSummary) =>
		`${c.connectionName}/${c.catalog}/${c.schema}/${c.cubeName}`;
...
<div class="pane">
	<header class="flex items-center justify-between">
		<div>
			...
			<p class="text-sm text-fg-muted" style="margin:2px 0 0">
				Governed AI-ask personas · prompt + hard cube allowlist
			</p>
		</div>
	...
	<div class="layout">
		<aside class="list">
			{#if spaces.length === 0}
				<p class="text-sm text-fg-muted" style="padding:var(--space-2)">
					No spaces yet. Create one, or drop JSON in <code>saiku-home/agent-spaces/</code>.
				</p>
			{/if}
			{#each spaces as s (s.id)}
				<button
					type="button"
					class="row"
					class:active={s.id === selectedId && !isNew}
					onclick={() => edit(s)}
				>
					<span class="nm">{s.name}</span>
					<span class="meta"
						>{s.cubeAllowlist.length} cube{s.cubeAllowlist.length === 1 ? '' : 's'}</span
					>
				</button>
			{/each}
		</aside>
		...
		{#if isNew || selectedId}
			<section class="editor">
				<div class="grid2">
					<label class="field">
						...
						<input
							bind:value={form.id}
							disabled={!isNew}
							placeholder="foodmart-sales-analyst"
							class:invalid={form.id !== '' && !idOk}
						/>
					</label>
					<label class="field">
						<span>Name</span>
				...
				<label class="field">
					<span
						>System prompt <small
							>(prepended to Saiku's built-in prompt — voice, focus, refusal rules)</small
						></span
					>
					<textarea
				...
				<div class="field">
					<span
						>Cube allowlist <small
							>(hard scope — an ask against any other cube is refused 403)</small
						></span
					>
					<div class="cubes">
						{#if cubes.length === 0}
							<p class="text-sm text-fg-muted">No cubes discovered.</p>
						{/if}
				...
				<div class="grid2">
					<label class="field">
						...
						<textarea bind:value={skillsText} rows="4" placeholder="weekly-foodmart-rollup"
						></textarea>
					</label>
					<label class="field">
						<span>Suggested prompts <small>(one per line)</small></span>
						<textarea bind:value={promptsText} rows="4" placeholder="How did sales track last week?"
						></textarea>
					</label>
				</div>
		...
		{:else}
			<section class="editor empty">
				<p class="text-sm text-fg-muted">Select a space to edit, or create a new one.</p>
			</section>
		{/if}
	</div>
</div>

...
<div class="pane">
		...
		<div class="flex items-center gap-2">
			{#if lastLoaded}
				<span class="text-sm text-fg-muted"
					>{i18n.t('admin.stats.updatedAt')} {fmtTime(lastLoaded)}</span
				>
			{/if}
			...
			<Button onclick={load} disabled={loading}>
				<RefreshCw size={14} /><span
					>{loading ? i18n.t('admin.stats.loading') : i18n.t('admin.stats.refresh')}</span
				>
			</Button>
		</div>
	</header>
	...
	{#if !stats}
		<p class="text-sm text-fg-muted">{i18n.t('admin.stats.idle')}</p>
	{:else}
		<section class="kpis">
			<div class="kpi">
				...
				<span class="text-xs text-fg-muted"
					>{fmtNum(stats.server.cellCacheHitCount)} / {fmtNum(
						stats.server.cellCacheRequestCount
					)}</span
				>
			</div>
			<div class="kpi">
				...
				<span class="text-xs text-fg-muted"
					>{i18n.t('admin.stats.kpi.avgDim')}: {fmtFloat(stats.avgCellDimensionality)}</span
				>
			</div>
			<div class="kpi">
				...
				<span class="text-xs text-fg-muted"
					>{i18n.t('admin.stats.kpi.created')}: {fmtNum(stats.server.segmentCreateCount)}</span
				>
			</div>
			<div class="kpi">
				<span class="kpi__label">{i18n.t('admin.stats.kpi.sql')}</span>
				<span class="text-2xl font-bold text-fg"
					>{fmtNum(stats.server.sqlStatementExecuteCount)}</span
				>
				<span class="text-xs text-fg-muted"
					>{i18n.t('admin.stats.kpi.cellRequests')}: {fmtNum(
						stats.server.sqlStatementCellRequestCount
					)}</span
				>
			</div>
			<div class="kpi">
				...
				<span class="text-xs text-fg-muted"
					>{stats.connections?.length ?? 0} {i18n.t('admin.stats.kpi.lifetime')}</span
				>
			</div>
			<div class="kpi">
				...
				<span class="text-xs text-fg-muted"
					>{i18n.t('admin.stats.kpi.executing')}: {stats.executingMdxStatementCount ?? 0}</span
				>
			</div>
		</section>
		...
		<section class="flex flex-col gap-2">
			...
			<p class="text-sm text-fg-muted">
				{stats.version?.productName ?? 'Mondrian'}
		...
		<section class="flex flex-col gap-2">
			<h3>
				{i18n.t('admin.stats.statements')}
				<span class="text-sm text-fg-muted">({stats.statements?.length ?? 0})</span>
			</h3>
			{#if !stats.statements?.length}
				<p class="text-sm text-fg-muted">{i18n.t('admin.stats.empty')}</p>
			{:else}
		...
		<section class="flex flex-col gap-2">
			<h3>
				{i18n.t('admin.stats.connections')}
				<span class="text-sm text-fg-muted">({stats.connections?.length ?? 0})</span>
			</h3>

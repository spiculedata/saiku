<script lang="ts">
	...
	const avgRate = $derived(
		trend.length ? trend.reduce((a, t) => a + t.passRate, 0) / trend.length : latestRate
	);
	...
	const linePts = $derived(
		trend.map((t, i) => `${xAt(i, trend.length)},${yAt(t.passRate)}`).join(' ')
	);
	const areaPts = $derived(
		trend.length
			? `${PL},${PT + IH} ${linePts} ${xAt(trend.length - 1, trend.length)},${PT + IH}`
			: ''
	);
...
<div class="pane">
	<header class="flex flex-wrap items-center justify-between gap-2">
		<div>
			<h2>Agent evals</h2>
			<p class="text-sm text-fg-muted" style="margin:2px 0 0">
	...
	{#if loaded && suites.length === 0}
		<div class="empty">
			<h3 style="text-transform:none;color:hsl(var(--fg));font-size:var(--fs-md)">
			...
			<p class="text-sm text-fg-muted">
				Author a suite (see <code>docs/EVAL-SPEC.md</code>) — prefer a <code>referenceQuery</code> so
				ground truth tracks the live data — then run the sweep. Each run appends a point here.
			</p>
		</div>
	{:else if suites.length}
		...
		<section class="kpis">
			<div class="kpi">
				...
				<span class="rate text-2xl font-bold {band(latestRate)}">{pct(latestRate)}</span>
			...
			<div class="kpi">
				...
				<span class="rate text-2xl font-bold {deltaPts >= 0 ? 'good' : 'bad'}">
		...
		<section class="flex flex-col gap-2">
			<h3>
				Pass-rate over time <span class="text-sm text-fg-muted">· each point is one run</span>
			</h3>
			{#if trend.length}
					...
					<svg viewBox="0 0 {W} {H}" width="100%" preserveAspectRatio="xMidYMid meet">
						...
						{#each gridLines as f (f)}
							<line
								...
								y2={PT + IH * (1 - f)}
							/>
							<text class="axt" x={PL - 8} y={PT + IH * (1 - f) + 3} text-anchor="end"
								>{(f * 100) | 0}%</text
							>
						{/each}
						...
						{#each trend as t, i (i)}
							<circle
								class="dot-{band(t.passRate)}"
								cx={xAt(i, trend.length)}
								cy={yAt(t.passRate)}
								r={i === trend.length - 1 ? 4 : 2.6}
							>
			...
			{:else}
				<p class="text-sm text-fg-muted">No runs for this suite yet.</p>
			{/if}
		</section>

		<section class="flex flex-col gap-2">
			<h3>Recent runs <span class="text-sm text-fg-muted">({runs.length})</span></h3>
			{#if runs.length}
				<div class="overflow-auto rounded-sm border border-border">
					<table>
						<thead>
							<tr>
								<th>When</th><th>Pass rate</th><th>Pass</th><th>Fail</th><th>Degraded</th><th
									>Cases</th
								><th>Elapsed</th>
							</tr>
						</thead>
			...
			{:else}
				<p class="text-sm text-fg-muted">No runs recorded.</p>
			{/if}
		</section>
	{/if}
</div>

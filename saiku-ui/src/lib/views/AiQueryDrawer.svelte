<script lang="ts">
	...
	async function submit(): Promise<void> {
		...
		if (resp.degraded) {
			...
			const text = isOffTopic
				? i18n
						.t('workspace.aiQuery.offTopic')
						.replace('{reason}', reason.slice('OFF_TOPIC: '.length))
				: reason;
			turns = [
				...turns,
				{
		...
		if (resp.emailDraft) {
			...
			turns = [
				...turns,
				{
					...
					role: 'assistant',
					text: i18n.t(
						'workspace.aiQuery.emailDraftOpening',
						'Opening a draft email for you to review…'
					),
					model: resp.model
				}
			];
		...
		if (resp.viewChange) {
			...
			const target =
				resp.viewChange.viewMode === 'chart'
		...
		const queryModel =
			resp.queryModel ??
			(resp.request ? aiRequestToQueryModel(resp.request as AiQueryRequestShape) : null);
		...
		const summary = i18n.t('workspace.aiQuery.resultSummary').replace('{rows}', String(rowCount));
		turns = [
			...turns,
			{
				id: nextId(),
	...
	async function submitChained(): Promise<void> {
		...
		const hydrateBuiltQuery = (env: AskResponse): void => {
			const queryModel =
				env.queryModel ?? aiRequestToQueryModel(env.request as AiQueryRequestShape);
			...
			await askAiStream(req, {
				...
				onFinal: (env) => {
					...
					if (kind === 'degraded') {
						...
						const target =
							env.viewChange.viewMode === 'chart'
								? `chart (${env.viewChange.chartType ?? '?'})`
								: 'grid';
						turns = [
							...turns,
							{
...
{#if open}
	<button
		type="button"
		class="fixed inset-0 z-[90] cursor-default border-0 bg-transparent p-0"
...
>
	...
	<div class="flex flex-1 flex-col gap-2.5 overflow-y-auto p-3.5" bind:this={scrollEl}>
		{#if turns.length === 0}
			<div class="ai-drawer__empty">
				...
				{#if selection.cube}
					<p class="ai-drawer__empty-cube">
						{i18n
							.t('workspace.aiQuery.activeCube')
							.replace('{cube}', selection.cube.caption ?? selection.cube.name)}
					</p>
		...
		{#each turns as turn (turn.id)}
			...
			{:else if turn.role === 'assistant'}
				<div class="ai-drawer__turn ai-drawer__turn--assistant">
					<div class="ai-drawer__bubble border border-border bg-bg-muted">
						...
						{/if}
						{#if turn.model}
							<div class="ai-drawer__model">
								{i18n.t('workspace.aiQuery.viaModel').replace('{model}', turn.model)}
							</div>
						{/if}
					</div>
				</div>
					...
					<div class="ai-drawer__bubble border border-border bg-bg-muted">
						...
						{#if turn.viewChange}
							<div class="mt-1 text-xs text-fg-subtle">
								{turn.viewChange.viewMode}{turn.viewChange.chartType
									? ` · ${turn.viewChange.chartType}`
									: ''}
							</div>
						{/if}
						{#if turn.model}
		...
		{#if inflight}
			<div class="ai-drawer__turn ai-drawer__turn--assistant">
				<div
					class="ai-drawer__bubble inline-flex items-center gap-2 border border-border bg-bg-muted"
				>
	...
	<footer class="flex shrink-0 flex-col gap-2 border-t border-border bg-bg px-3.5 py-2.5">
		...
		<div
			class="ai-drawer__mode-bar"
			role="radiogroup"
			aria-label={i18n.t('workspace.aiQuery.modeLabel')}
		>
			{#each [{ id: 'auto', label: i18n.t('workspace.aiQuery.modeAuto') }, { id: 'query', label: i18n.t('workspace.aiQuery.modeQuery') }, { id: 'insight', label: i18n.t('workspace.aiQuery.modeInsight') }, { id: 'view_change', label: i18n.t('workspace.aiQuery.modeView') }] as opt (opt.id)}
				<button
					...
					class={'ai-drawer__mode-btn ' +
						(forceTool === opt.id ? 'ai-drawer__mode-btn--active' : '')}
					disabled={inflight}
					onclick={() => (forceTool = opt.id as ForceTool)}
					title={i18n.t(`workspace.aiQuery.modeTip.${opt.id}`)}>{opt.label}</button
				>
			{/each}
		</div>
...
<style>
	...
	.ai-drawer__insight-body :global(h4:first-child) {
		margin-top: 0;
	}
	.ai-drawer__insight-body :global(p) {
		margin: 0 0 8px;
	}
	.ai-drawer__insight-body :global(p:last-child) {
		margin-bottom: 0;
	}

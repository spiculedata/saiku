<script lang="ts">
	...
	async function runRowDetail() {
		...
		try {
			...
			toasts.success(
				`${resp.records.length} rows fetched`,
	 ...
	 */
	function currentAggregationFor(
		metricName: string
	): import('$lib/api/ossie').OssieAggregation | null {
		...
		<div class="relative flex items-center gap-0.5" role="group" aria-label="Export">
			...
			{#if exportMenuOpen}
				<div class="toolbar__dropdown right-0 left-auto">
					...
					<button type="button" class="toolbar__item" disabled title="XLS export coming next">
						<FileSpreadsheet size={16} /> <span>XLS (coming soon)</span>
					</button>
					<button type="button" class="toolbar__item" disabled title="PDF export coming next">
			...
			{#each ossieQuery.current?.rows ?? [] as f, i}
				...
				>
					...
					<button
						...
						onclick={() => ossieQuery.removeRow(i)}><X size={12} /></button
					>
				</span>
			{/each}
		</div>
			...
			{#each ossieQuery.current?.columns ?? [] as f, i}
				...
				>
					...
					<button
						...
						class="ossie-chip__x"
						aria-label="Remove {f.field}"
						onclick={() => ossieQuery.removeColumn(i)}><X size={12} /></button
					>
				</span>
			{/each}
		</div>
			...
			{#each ossieQuery.current?.values ?? [] as v, i}
				...
				>
					...
					<button
						...
						onclick={() => ossieQuery.removeValue(i)}><X size={12} /></button
					>
				</span>
			{/each}
		</div>
				...
				<span class="ossie-chip ossie-chip--filter">
					...
					<select
						...
						onchange={(e) =>
							updateFilterOp(
								i,
								(e.currentTarget as HTMLSelectElement).value as OssieFilterExpr['op']
							)}
					>
					...
					{#if f.op === 'BETWEEN'}
						<input
							...
							value={f.values?.[0] ?? ''}
							oninput={(e) =>
								updateFilterValueAt(i, 0, (e.currentTarget as HTMLInputElement).value)}
							placeholder="from"
						/>
						...
						<input
							...
							value={f.values?.[1] ?? ''}
							oninput={(e) =>
								updateFilterValueAt(i, 1, (e.currentTarget as HTMLInputElement).value)}
							placeholder="to"
						/>
					{:else if f.op === 'IN'}
						{#each f.values ?? [] as v, vi (vi)}
							<span class="ossie-chip__in-slot">
								<input
									...
									value={v}
									oninput={(e) =>
										updateFilterValueAt(i, vi, (e.currentTarget as HTMLInputElement).value)}
									placeholder="value"
								/>
								<button
									type="button"
									...
									onclick={() => removeFilterValue(i, vi)}><X size={10} /></button
								>
							</span>
						{/each}
						<button
							...
							onclick={() => addFilterValue(i)}>+</button
						>
					...
					<button
						...
						onclick={() => ossieQuery.removeFilter(i)}><X size={12} /></button
					>
				</span>
			{/each}
		</div>
				...
				{#each ossieQuery.current.columns as colRef, colIdx (colIdx)}
					...
					{#if val !== undefined}
						<button
							type="button"
							class="ossie-ctx-menu__item"
							onclick={() => ctxCrosstabFilter(colIdx)}
						>

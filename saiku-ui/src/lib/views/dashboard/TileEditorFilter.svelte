...
<label class="field">
	...
	<select
		class="field__input"
		bind:value={filterTarget.hierarchy}
		disabled={!filterTarget.dimension}
	>
		<option value="">— pick —</option>
...
{#if widget === 'cascading-select'}
	<fieldset class="size">
		...
		<label class="field flex-1">
			...
			<select
				class="field__input"
				bind:value={cascadeStartLevel}
				disabled={!filterTarget.hierarchy}
			>
	...
	<span class="hint">
		Renders one dropdown per level from the start level down. Picking a parent reveals its children;
		choosing &ldquo;All&rdquo; resets the levels below.
	</span>
{/if}

...
{#if open}
	...
	<div class="modal" role="dialog" aria-modal="true" aria-label="Suggest filter widgets">
		<header class="head">
			...
			<button
				type="button"
				class="cursor-pointer border-0 bg-transparent text-xl leading-none text-fg-muted"
				aria-label="Close"
		...
		{:else if suggestions.length === 0}
			<p class="empty">
				Nothing to suggest — your tiles don't use any dimensions yet, or every candidate already has
				a filter widget on the dashboard.
			</p>
		{:else}
			<p class="hint">
				These dimensions/levels are used by your dashboard's chart and table tiles. Uncheck the ones
				you don't want to expose as a filter widget.
			</p>
			<div class="bulk">
			...
			<ul class="m-0 min-h-0 flex-1 list-none overflow-y-auto px-0 py-2">
		...
		<footer class="flex justify-end gap-2 border-t border-border px-4 py-3">
			...
			<Button onclick={handleAdd} disabled={suggestions.length === 0}>Add selected</Button>
		</footer>
	</div>
{/if}

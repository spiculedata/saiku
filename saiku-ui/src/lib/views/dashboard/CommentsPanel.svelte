<script lang="ts">
	...
	import {
		...
		currentMentionToken,
		matchUsers
	} from '$lib/dashboard/mentionAutocomplete';
...
<Modal title={tileTitle ? `Comments — ${tileTitle}` : 'Comments'} {open} size="md" {onClose}>
	<div class="cmts">
		...
		{:else}
			<ul class="cmts__list">
				{#each comments as c (c.id)}
					<li class="rounded-sm border border-border bg-bg-subtle p-2">
						<div class="flex items-center gap-2">
							<span class="text-sm font-bold">{c.author}</span>
		...
		<div class="flex items-end gap-2 border-t border-border pt-2">

<script lang="ts">
	...
	async function restore(version: string): Promise<void> {
		if (
			!confirm('Restore this version? The current state is archived first, so this is reversible.')
		) {
			return;
		}
		busyVersion = version;
		...
		{:else}
			<ul class="m-0 flex list-none flex-col p-0">
				{#each versions as v (v.version)}
					<li class="flex items-center justify-between gap-3 border-b border-border px-0 py-2">

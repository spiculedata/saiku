...
{#if kpiConfig.format === 'custom'}
	<label class="field">
		...
		/>
		<span class="hint">
			$N / €N / £N for currency, $cN for compact currency ($48.2K), N% for percent, plain N for
			fractional digits.
		</span>
	</label>
{/if}
	...
	{#if kpiConfig.comparison !== 'none'}
		<label class="field flex-1">
			...
			<input
				class="field__input"
				type="text"
				bind:value={kpiConfig.deltaSuffix}
				placeholder="vs prior"
			/>
...
{#if showTimeLevel}
	<fieldset class="size">
		...
		<label class="field flex-1">
			...
			<select
				class="field__input"
				bind:value={kpiConfig.timeLevel!.dimension}
				disabled={!cubePicked}
			>

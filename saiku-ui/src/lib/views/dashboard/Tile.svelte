...
>
	...
	>
		<span class="flex-1 overflow-hidden font-medium text-ellipsis whitespace-nowrap"
			>{tile.title ?? defaultTitle(tile)}</span
		>
	...
	<div class="min-h-0 flex-1 overflow-auto">
		...
		{:else if tile.type === 'custom'}
			...
			{:else}
				<div class="p-2 text-sm text-fg-muted">
					Unknown renderer: {tile.custom?.renderer ?? '(none)'}
				</div>
			{/if}
		{:else}
			<div class="p-2 text-sm text-danger">Unknown tile type: {tile.type}</div>
		{/if}
	</div>
</div>

{#if menuOpen}
	<div class="tile-menu" role="menu" bind:this={menuEl} style="left: {menuX}px; top: {menuY}px;">
		...
		<div class="tile-menu__row" role="group" aria-label="Tile height">
			<span class="flex-1 text-sm text-fg-muted">Height</span>
			<button
				type="button"

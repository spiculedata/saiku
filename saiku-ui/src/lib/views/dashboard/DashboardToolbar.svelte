<script lang="ts">
	...
	async function runExport(format: ExportFormat): Promise<void> {
		...
		exportError = null;
		try {
			await exportDashboard(gridElement, { title: name, filters: activeFilters.all }, format);
		} catch (err) {
...
<header
	...
	bind:this={headerEl}
>
	<div class="title-block flex min-w-0 flex-col gap-1">
		{#if readOnly}
			<h1 class="dash-title m-0 text-lg font-semibold">{name}</h1>
		{:else}
			<input
				class="name"
				type="text"
		...
		{#if readOnly}
			{#if tags.length > 0}
				<div class="flex flex-wrap items-center gap-1 pl-2" aria-label="Dashboard tags">
					{#each tags as t (t)}
...
{#snippet primaryActions()}
	...
	{#if !readOnly}
		<div
			class="undo-redo"
			role="group"
			aria-label={i18n.t('dashboard.history.group', 'Undo and redo')}
		>
			<Button
				variant="outline"
				class="icon-only"
	...
	<div class="relative inline-flex">
		...
		>
			...
			<span
				>{exporting
					? i18n.t('dashboard.export.busy', 'Exporting…')
					: i18n.t('dashboard.export', 'Export')}</span
			>
		</Button>
		{#if exportMenuOpen}
			<div
				class="export-menu"
				role="menu"
				aria-label={i18n.t('dashboard.export.formatLabel', 'Export format')}
			>
				<Button
					variant="outline"
					class="export-option"
...
{#if exportError}
	<div class="export-error" role="alert">
		...
		<button
			type="button"
			class="inline-flex cursor-pointer items-center border-0 bg-none p-0 text-inherit"

<script lang="ts">
	...
	import { ancestorSelections, memberDescendsFromAny } from '$lib/dashboard/cascadingFilters';
	...
	import { expandDateRange, fromDateInputValue, toDateInputValue } from '$lib/dashboard/dateRange';
	import {
		listAiCubes,
		type AiCubeSummary,
		type CubeRef,
	...
	function levelIndex(schema: SchemaLike | null, dim: string, hier: string, level: string): number {
	...
	function displayedOptionsFor(f: PanelFilter): { uniqueName: string; caption: string }[] | null {
		const cat = memberCatalogues[f.id];
	...
	function handleCascadeCommit(filterId: string, next: CascadeSelections, members: string[]): void {
	...
	<section class="panel" aria-label="Dashboard filter panel">
		...
		{#if !collapsed}
			{#if (panel?.filters.length ?? 0) === 0 && !adding}
				<p class="empty">
					No filters yet — click <em>+ Add filter</em> to register one, or use
					<em>Suggest filters</em> in the toolbar.
				</p>
			{/if}
			{#if (panel?.filters.length ?? 0) > 0}
				...
				>
					{#each panel?.filters ?? [] as f (f.id)}
                     ...
                     dashboard never sees the concept. -->
								<details
									class="bindings"
									ontoggle={() => ossieSources.forEach(ensureOssieCatalogue)}
								>
                     ...
                     pick can flow back through commitPanelMembers. -->
								<CascadingFilterPicker
									...
									readOnly={false}
									onCommit={(next, members) => handleCascadeCommit(f.id, next, members)}
								/>
							{:else if f.widget === 'date-range'}
								...
								<span class="inline-flex items-center gap-1">
									<input
										...
										aria-label="From"
										onchange={(e) =>
											setDateRange(f.id, (e.target as HTMLInputElement).value, dr.to)}
									/>
									<span class="text-xs text-fg-muted" aria-hidden="true">→</span>
									<input
										type="date"
										class="picker-date"
										value={dr.to}
										aria-label="To"
										onchange={(e) =>
											setDateRange(f.id, dr.from, (e.target as HTMLInputElement).value)}
									/>
								</span>
							{:else if f.widget === 'top-n'}
								...
								<span class="topn">
									<select
										...
										onchange={(e) =>
											setTopN(f.id, {
												direction: (e.target as HTMLSelectElement).value as 'top' | 'bottom'
											})}
									>
									...
									<input
										...
										aria-label="N"
										onchange={(e) =>
											setTopN(f.id, { n: Number((e.target as HTMLInputElement).value) })}
									/>
									...
									<select
										...
										aria-label="Measure"
										onchange={(e) =>
											setTopN(f.id, { measure: (e.target as HTMLSelectElement).value })}
									>
			...
			{#if adding}
				<form
					class="flex flex-wrap items-end gap-2 border-t border-border bg-bg-muted p-2"
					onsubmit={(e) => {
						e.preventDefault();
						commitAdd();
					}}
					...
					<label class="add-field">
						...
						<select onchange={handleAddCubeChange} value={addCube ? cubeKey(addCube) : ''}>
							...
							{#each cubes as c (cubeKey(c))}
								<option value={cubeKey(c)}
									>{c.cubeCaption ?? c.cubeName} ({c.connectionName})</option
								>
							{/each}
						</select>
					</label>
   ...
   a plain dashboard is unchanged. */
	...
	.picker--hover {
		box-shadow: 0 0 0 2px
			var(--accent, color-mix(in srgb, var(--saiku-app-fg, hsl(var(--fg))) 30%, transparent));
	}
	.grip {
		cursor: grab;
		color: var(--saiku-app-muted, hsl(var(--fg-muted)));

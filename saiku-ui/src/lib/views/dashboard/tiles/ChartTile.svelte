<script lang="ts">
	...
	function anomalyTooltip(m: AnomalyMark): string {
		...
		const headline = dirWord
			? `${sigma}σ ${escapeHtml(dirWord)} ${escapeHtml(sigmaSuffix)}`
	...
	function handleBrushSelected(params: unknown): void {
		...
		void Promise.all(captions.map((c) => resolveMemberUniqueName(cube, rowAxis, c))).then(
			(names) => {
...
<style>
	...
	.overlay {
       ...
       is the fallback for a plain dashboard, where nothing changes. */
		background: color-mix(in srgb, var(--saiku-app-surface, hsl(var(--bg))) 75%, transparent);

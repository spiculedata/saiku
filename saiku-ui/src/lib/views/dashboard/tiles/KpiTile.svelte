<script lang="ts">
	...
	let kpi = $derived<
		Required<Pick<KpiConfig, 'format' | 'comparison' | 'direction' | 'sparkline'>> & KpiConfig
	>({
		format: 'number',
		comparison: 'none',
	...
	function fetchLevelMembers(
		...
		const promise = fetch(`/rest/saiku/api/ai/members/search?${params.toString()}`, {
			...
			.then((r) => (r.ok ? r.json() : []))
			.then((hits: unknown) =>
				Array.isArray(hits) ? (hits as { uniqueName: string; caption: string }[]) : []
			)
		...
		{:else}
			...
			{:else if kpi.comparison === 'year-over-year' && wantsSeries && valueAndPrior.current != null && valueAndPrior.prior == null}
				<div
					...
					title={i18n.t(
						'dashboard.kpi.noLastYear.title',
						"The selected period has no same-period-previous-year counterpart in this cube's data."
					)}
				>
			...
			{/if}
			{#if kpi.sparkline && wantsSeries}
				<div class="h-[36px] w-full shrink-0" bind:this={sparkHost} aria-hidden="true"></div>
			{/if}
		{/if}

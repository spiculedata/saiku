...
.table-tile {
  ...
  /* saiku#1802: subtotal rows. Weight and a faint tint, not a colour — the row
     ...
     light preset the page ground is the same colour as the card, so the marking
     resolved to pure white on white and the fix was invisible on screen while
     passing its test. Deriving from the foreground guarantees contrast against
     whatever the card is, in any theme. */
  tr.rollup > td {
    font-weight: var(--weight-semibold);

<script lang="ts">
	...
	import { isAiCell, searchMembers, type AiCell } from '$lib/api/aiQuery';
	...
	import { formatCell, cellFormatToStyle, type CellFormat } from '$lib/dashboard/conditionalFormat';
		...
		{:else if response && response.status === 'SUCCESS'}
			...
			{#if rows.length === 0}
				<TileEmpty
					...
					cubeName={resolvedCube?.cubeName}
				/>
			{:else}
				<table>
					<thead>
                   ...
                   the one tile where a subtotal earns its place. -->
                       ...
                       button role + Enter/Space keyboard activation. -->
									<td
										...
										oncontextmenu={(e) => handleCellContextMenu(e, col.caption, col.isRowHeader, i)}
									...
									>
										{#if cf.icon}<span class="mr-1 inline-block font-semibold" aria-hidden="true"
												>{cf.icon}</span
											>{/if}{fmt.display}
									</td>
								{/each}

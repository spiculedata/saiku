<script lang="ts">
	...
	let hasConfig = $derived(!!tile.custom?.options && Object.keys(tile.custom.options).length > 0);
...
{:else if !validation.ok}
	<div class="p-3 text-sm text-danger" role="alert">
		<div class="font-medium">Invalid graph config</div>
		<div class="mt-1">{validation.error}</div>
	</div>
{:else if !tile.query}
	<div class="p-3 text-sm text-fg-muted">Tile has no query binding — open ⚙ to set one.</div>
{:else}
	<div class="ec-tile">

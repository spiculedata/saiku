...
<Modal title="Share dashboard" {open} size="md" {onClose}>
	<div class="share">
		<p class="m-0 text-sm text-fg-muted">
			Anyone with the link can <strong>view</strong> this dashboard read-only — no Saiku account needed.
		...
		{#if error}<p class="m-0 text-sm text-danger">{error}</p>{/if}
		...
		{#if tokens.length > 0}
			<div class="share__list">
				<span class="text-sm font-bold text-fg-muted">Active links</span>

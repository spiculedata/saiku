...
{#snippet dashboardRow(relPath: string, label: string)}
	...
	<Button
		variant="outline"
		class="icon-only star {isFav ? 'star--on' : ''}"
...
{#snippet folderBranch(node: FolderNode, depth: number)}
	...
	<li class="tree-folder" style="--depth: {depth}">
		...
		>
			<span class="inline-flex flex-none items-center text-fg-muted" aria-hidden="true">
				{#if open}
					<ChevronDown size={14} />
				{:else}
			...
			<span class="tree-folder-count">{node.folders.length + node.dashboards.length}</span>
...
<ul
	class="tree m-0 flex list-none flex-col gap-1 p-0"
	aria-label={i18n.t('dashboard.view.folders')}
>

<script lang="ts">
  ...
  const cubeId = $derived(
    `${cube.connectionName}/${cube.catalog}/${cube.schema}/${cube.cubeName}`,
  );
  ...
  const levels = $derived.by((): string[] => {
    ...
    const startIdx = allLevels.findIndex(
      (n) => n.toLowerCase() === start.toLowerCase(),
    );
  ...
  async function loadLevel(
    ...
    try {
      ...
      const res = await fetch(
        `/rest/saiku/api/ai/members/search?${params.toString()}`,
  ...
  function membersFor(levelIdx: number): MemberRowState | undefined {
    const parent =
      levelIdx === 0 ? null : parentForLevel(selections, levelIdx - 1);

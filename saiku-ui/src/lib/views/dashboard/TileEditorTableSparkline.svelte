<script lang="ts">
	...
	let { sparklineEnabled = $bindable(), sparklineType = $bindable() }: Props = $props();
</script>

<fieldset class="cf-section">
	<legend>Sparkline column</legend>
	<span class="hint">
		Adds a trailing &ldquo;Trend&rdquo; column drawing a mini chart per row from that row's numeric
		measure values. Needs at least two measure columns to render; rows with fewer numeric values
		show a dash.
	</span>

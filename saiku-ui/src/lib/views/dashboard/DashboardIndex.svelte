<script lang="ts">
	...
	import { LayoutDashboard, Star, FolderInput, MoreVertical } from '@lucide/svelte';
	import {
	...
	let folderTree = $derived<FolderNode>(
		buildFolderTree(
			filteredEntries.map((e): CatalogueLeaf => ({
	...
	async function onRenameFolder(newName: string): Promise<void> {
		...
		try {
			...
			toasts.danger(i18n.t('toast.renameFailed'), e instanceof Error ? e.message : String(e));
		} finally {
			movingBusy = false;
		}
	}

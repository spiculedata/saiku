<script lang="ts">
	...
	let brushCrossFilterEnabled = $state<boolean>(
		untrack(() => tile.brushCrossFilter?.enabled ?? false)
	);
	...
	let referencePath = $state<string>(
		untrack(() => (tile.query?.kind === 'reference' ? tile.query.path : ''))
	);
	...
	const isGraphTile = untrack(() => tile.type === 'custom' && tile.custom?.renderer === 'graph');
	...
	const isPluginTile = untrack(() => tile.type === 'custom' && tile.custom?.renderer === 'plugin');
	...
	let rankedLimit = $state<number>(
		untrack(() => (typeof savedRanked.limit === 'number' ? savedRanked.limit : DEFAULT_LIMIT))
	);
	...
	let rankedValueFormat = $state<string>(untrack(() => String(savedRanked.valueFormat ?? '')));
	...
	let selectedPluginId = $state<string>(
		untrack(() =>
			typeof tile.custom?.options?.pluginId === 'string' ? tile.custom.options.pluginId : ''
		)
	);
	...
	async function openQueryEditor(): Promise<void> {
		...
		seedBody = tile.query?.kind === 'inline' ? (tile.query.body as InlineQueryBody) : null;
	...
	async function handleSave(): Promise<void> {
		...
		if (tile.type === 'chart') {
				...
				anomalyEnabled && ANOMALY_CHART_KINDS.has(chartType)
					? {
							...
							threshold:
								...
								Number.isFinite(anomalyThreshold) &&
								anomalyThreshold > 0
									? anomalyThreshold
									: undefined,
			...
			patch.brushCrossFilter =
				brushCrossFilterEnabled && BRUSHABLE_CHART_TYPES.has(chartType)
					? { enabled: true }
					: undefined;
		}
		...
		if (tile.type === 'kpi') {
			...
			const cleaned: KpiConfig = {
				...kpiConfig,
				timeLevel: needsTime && tl && tl.dimension && tl.hierarchy && tl.level ? tl : undefined,
		...
		if (tile.type === 'table') {
			...
			patch.columnFormats =
				Object.keys(columnFormats).length > 0 ? { ...columnFormats } : undefined;
...
>
	<div class="modal" class:modal--wide={queryEditorOpen} role="dialog" aria-label="Edit tile">
		<header class="modal-header">
			...
			<button
				type="button"
				class="cursor-pointer border-0 bg-transparent text-xl text-fg-muted"
				aria-label="Close"
		...
		<div class="flex flex-col gap-3 overflow-auto p-4">
			<label class="field">
				...
				<input
					...
					type="text"
					bind:value={title}
					placeholder={`Untitled ${tile.type}`}
				/>
			</label>
							...
							{#if ossieModels.length > 0}
								<optgroup label="Semantic models (SQL)">
									{#each ossieModels as m (ossieKey(m))}
										<option
											value={ossieKey(m)}
											selected={cube ? ossieKey(m) === cubeKey(cube) : false}
										>
					...
					<fieldset class="anomaly">
                     ...
                     pick a method that 400s. Re-add when StlAnomalyDetector is implemented. -->
								<select class="field__input" bind:value={anomalyMethod}>
									<option value="zscore"
										>{i18n.t('dashboard.anomaly.method.zscore', 'Z-score')}</option
									>
									<option value="mad"
										>{i18n.t('dashboard.anomaly.method.mad', 'MAD (robust)')}</option
									>
								</select>
							</label>
							<label class="field">
								<span class="field__label"
									>{i18n.t('dashboard.anomaly.threshold', 'Threshold')}</span
								>
								<input
									class="field__input"
									type="number"
								...
								<span class="hint"
									>{i18n.t(
										'dashboard.anomaly.threshold.hint',
										'Sigmas from expectation. Blank uses the method default.'
									)}</span
								>
							</label>
							<label class="field">
								...
								<input
									...
									placeholder={i18n.t(
										'dashboard.anomaly.timeAxis.placeholder',
										'Defaults to the first row axis'
									)}
								/>
								<span class="hint"
									>{i18n.t(
										'dashboard.anomaly.timeAxis.hint',
										'Unique name of the time level, e.g. [Time].[Time].[Month].'
									)}</span
								>
							</label>
						{/if}
					...
					<fieldset class="anomaly">
                     ...
                     Re-add when ArimaForecaster / ProphetForecaster are implemented. -->
								<select class="field__input" bind:value={forecastMethod}>
									<option value="ets"
										>{i18n.t('dashboard.forecast.method.ets', 'Exponential smoothing')}</option
									>
								</select>
							</label>
							<label class="field">
								<span class="field__label"
									>{i18n.t('dashboard.forecast.horizon', 'Horizon (points)')}</span
								>
								<input
									class="field__input"
									type="number"
								...
								<span class="hint"
									>{i18n.t(
										'dashboard.forecast.horizon.hint',
										'How many future points to project.'
									)}</span
								>
							</label>
							<label class="field">
								<span class="field__label"
									>{i18n.t('dashboard.forecast.confidence', 'Confidence')}</span
								>
								<input
									class="field__input"
									type="number"
									...
									max="0.999"
									step="0.01"
									bind:value={forecastConfidence}
								/>
								<span class="hint"
									>{i18n.t(
										'dashboard.forecast.confidence.hint',
										'Prediction-interval level, e.g. 0.95.'
									)}</span
								>
							</label>
							<label class="field">
								<span class="field__label"
									>{i18n.t('dashboard.forecast.timeAxis', 'Time axis')}</span
								>
								<input
									class="field__input"
									type="text"
									...
									placeholder={i18n.t(
										'dashboard.forecast.timeAxis.placeholder',
										'Defaults to the first row axis'
									)}
								/>
								<span class="hint"
									>{i18n.t(
										'dashboard.forecast.timeAxis.hint',
										'Unique name of the time level, e.g. [Time].[Time].[Month].'
									)}</span
								>
							</label>
           ...
           validation. Persisted into tile.custom.options on Save. -->
			{#if isEchartsOptionTile}
				<label class="field">
					...
					{/if}
					<span class="hint">
						Declarative ECharts option only — functions and remote URLs are rejected. Categories +
						series data come from the tile's query below.
					</span>
				</label>
				<label class="checkbox">
				...
				<span class="hint">
					$c0 / €c1 for compact currency ($149K), $2 for plain currency, 1% for percent, 0 for
					fractional digits. Numeric axis formatting needs a function, which author options can't
					contain — so it's stated here and compiled at render time.
				</span>
			{/if}
				...
				<fieldset class="graph-map">
					...
					<label class="field">
						...
						<input
							class="field__input"
							type="text"
							bind:value={graphIdCol}
							spellcheck="false"
							placeholder="node id (often = source)"
						/>
					</label>
					<label class="field">
						...
						<input
							class="field__input"
							type="text"
							bind:value={graphLabelCol}
							spellcheck="false"
							placeholder="display name for the id node"
						/>
					</label>
					<label class="field">
						...
						<input
							class="field__input"
							type="text"
							bind:value={graphValueCol}
							spellcheck="false"
							placeholder="measure carried onto edges"
						/>
					</label>
					<label class="field">
					...
					{#if graphConfigValidation.ok}
						<span class="hint ok"
							>✓ Valid mapping — nodes + edges are built from the tile's query below.</span
						>
					{:else}
						<span class="hint error">{graphConfigValidation.error}</span>
					{/if}
					<span class="hint">
						Each query row becomes an edge source → target; nodes are deduped across rows. Column
						names must match the query's row-header / measure captions.
					</span>
				</fieldset>
			{/if}
				...
				<fieldset class="graph-map">
					...
					<label class="field">
						...
						<input
							class="field__input"
							type="text"
							bind:value={rankedSubtitle}
							placeholder="e.g. Product department · MoM"
						/>
					</label>
					<label class="field">
						<span class="field__label">Label column <span class="hint">(optional)</span></span>
						<input
							class="field__input"
							type="text"
							bind:value={rankedLabelCol}
							spellcheck="false"
							placeholder="blank = inferred"
						/>
					</label>
					<label class="field">
						<span class="field__label">Value column <span class="hint">(optional)</span></span>
					...
					{/if}
					<span class="hint"> Colours and type follow the app theme — no custom CSS needed. </span>
				</fieldset>
			{/if}
           ...
           Persisted into tile.custom.options.pluginId on Save. -->
			{#if isPluginTile}
				<label class="field">
					...
					{/if}
					<span class="hint">
						Runs admin-installed, trusted plugin code in a sandboxed iframe. It cannot read the host
						page, cookies, or session token; query data (below) is delivered as records.
					</span>
				</label>
			{/if}
             ...
             ════════════════════════════════════════════════════════════ -->
				{#if !queryEditorOpen}
					<div class="qe-launch">
						...
						<span class="hint">
							{cube
								? i18n.t('tileEditor.query.editInline.hint')
								: i18n.t('tileEditor.query.pickCubeFirst')}
						</span>
					</div>
					{#if queryEditorError}
				...
				{:else}
					<fieldset class="qe-section">
						...
						<div class="qe-embed">
							{#if session.current}
								<aside class="min-w-0 overflow-y-auto border-r border-border bg-bg-subtle p-2">
					...
					{#if queryMode === 'reference'}
						<label class="field">
							...
							{:else if savedQueries.length === 0}
								<span class="hint">
									No saved queries in the repository yet — switch to Inline JSON, or save a query
									from the main workspace first.
								</span>
							{:else}
								...
								<span class="hint">
									The tile renders live from the saved query — the resolver (POST /ai/query/saved)
									loads it, merges any applicable dashboard filters, and runs it on each refresh.
								</span>
							{/if}
						</label>
					{:else}
						<label class="field">
							...
							{:else}
								<span class="hint">
									Paste an AiQueryRequest, or use “{i18n.t('tileEditor.query.editInline')}” above to
									build it visually.
								</span>
							{/if}
						</label>
...
<style>
	...
	.modal :global(.hint.error) {
		color: hsl(var(--danger));
	}
	.modal :global(.hint.ok) {
		color: var(--success, hsl(var(--primary)));
	}

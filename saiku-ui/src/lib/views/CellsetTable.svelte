...
{:else}
			...
			<thead>
				{#each parsed.columnHeaderRows as hdrRow, rIdx}
					<tr aria-rowindex={rIdx + 1}>
						{#each hdrRow as c, cIdx}
							...
							{:else if c.type === 'COLUMN_HEADER'}
								...
								<th
									...
									title={c.value}
									oncontextmenu={(e) => openMenu(e, c, 'COLUMNS')}>{c.value || '\u00A0'}</th
								>
							{:else}
						...
						{#if spark !== 'none'}
							<th
								class={rIdx === parsed.columnHeaderRows.length - 1
									? 'col col--last spark-col'
									: 'col spark-col'}
								role="columnheader"
								aria-colindex={hdrRow.length + 1}
							>
								{rIdx === parsed.columnHeaderRows.length - 1
									? spark === 'bar'
										? 'Sparkbar'
										: 'Sparkline'
									: '\u00A0'}
			...
			<tbody>
				{#if renderWindow.topPad > 0}
					<tr class="virt-spacer" aria-hidden="true"
						><td
							colspan={parsed.rowHeaderColCount +
								(parsed.dataRows[0]?.length ?? 0) +
								(spark !== 'none' ? 1 : 0)}
							style="height: {renderWindow.topPad}px; padding: 0; border: 0;"
						></td></tr
					>
				{/if}
				{#each parsed.bodyRows.slice(renderWindow.first, renderWindow.last + 1) as rowCells, iOffset}
					...
					<tr class="vrow" aria-rowindex={parsed.columnHeaderRows.length + r + 1}>
						...
						{#if spark !== 'none'}
							...
							<td
								class="data spark-cell"
								role="gridcell"
								aria-colindex={parsed.rowHeaderColCount + (parsed.dataRows[r]?.length ?? 0) + 1}
							>
								<svg
								...
								>
									{#if spark === 'line'}
										<path
											d={linePath(vals, range.min, range.max, SPARK_W, SPARK_H)}
					...
					<tr class="virt-spacer" aria-hidden="true"
						><td
							colspan={parsed.rowHeaderColCount +
								(parsed.dataRows[0]?.length ?? 0) +
								(spark !== 'none' ? 1 : 0)}
							style="height: {renderWindow.bottomPad}px; padding: 0; border: 0;"
						></td></tr
					>
				{/if}
			</tbody>
	...
	{#if selStats}
		<div class="sel-stats" role="status" aria-live="polite">
			<span
				>{i18n.t('stats.selected')}: <strong>{selStats.cells}</strong>
				{selStats.cells === 1 ? i18n.t('stats.cell') : i18n.t('stats.cells')}</span
			>
			...
			<button
				type="button"
				class="sel-stats__clear"
				onclick={clearSelection}
				title={i18n.t('stats.clearSelection')}>×</button
			>
		</div>
	{/if}
	{#if result.runtime != null}
		<p class="runtime">
			{i18n.t('stats.runtime')}: {result.runtime}
			{i18n.t('units.ms')} · {result.height ?? 0}
			{i18n.t('units.rows')} × {result.width ?? 0}
			{i18n.t('units.cols')}
		</p>
	{/if}
{/if}

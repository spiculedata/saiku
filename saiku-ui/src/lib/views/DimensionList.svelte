<script lang="ts">
	...
	import CalculatedMemberModal, {
		type CalculatedMember
	} from '$lib/modals/CalculatedMemberModal.svelte';
	...
	function onCalculatedSave(m: CalculatedMember): void {
		...
		toasts.success(
			i18n.t('toast.calcMeasure'),
			i18n.t('toast.calcMeasure.body').replace('{name}', m.name)
		);
	}
	...
	async function openDimensionFilter(
		...
		try {
			...
			try {
				members = await listRootMembers(session.current.username, selection.cube, hier.uniqueName);
			} catch (err) {
				toasts.danger(
		...
		<section class="panel">
			<header class="panel__header flex items-center justify-between">
				...
				<span class="inline-flex gap-1">
					<button
						type="button"
						class="panel__action"
						title={i18n.t('panels.manageMeasures')}
						aria-label={i18n.t('panels.manageMeasures')}
					...
					<button
						type="button"
						class="panel__action"
						title={i18n.t('panels.newCalcMeasure')}
						aria-label={i18n.t('panels.newCalcMeasure')}
			...
			<ul class="tree">
						...
						<li class="tree__node">
							<span class="tree__row tree__row--measure">
								<button
									...
									onclick={(e) => {
										if (e.detail === 0) onMeasureAdd(measure);
									}}
								>
									<span class="tree__icon tree__icon--measure" aria-hidden="true">
										{#if measure.calculated}<FunctionSquare size={13} />{:else}<Sigma
												size={13}
											/>{/if}
									</span>
									<span class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap"
										>{measure.caption || measure.name}</span
									>
								</button>
				...
				{#if metadata.measures.length === 0}
					<li class="p-2 text-sm text-fg-subtle">{i18n.t('panels.noMeasures')}</li>
				{/if}
			</ul>
		</section>
		...
		<section class="panel">
			...
			<ul class="tree">
											...
											<li class="tree__node">
												<span class="tree__row tree__row--level">
													<button
														...
														onclick={(e) => {
															if (e.detail === 0) onLevelAdd(dim, hier, lvl);
														}}
													>
														<span class="tree__icon text-fg-subtle" aria-hidden="true"
															><Minus size={11} /></span
														>
														<span class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap"
															>{lvl.caption || lvl.name}</span
														>
													</button>
                               ...
                               dropped onto ROWS first if not on the query yet. -->
									...
									{:else}
										<li class="tree__node">
											<button
												type="button"
												class="tree__row text-fg-subtle"
												onclick={() => toggle(hid)}
											>
												<span
													class="tree__twisty"
													class:tree__twisty--open={expanded[hid] !== false}
												>
													<ChevronRight size={12} />
												</span>
												<span class="tree__icon" aria-hidden="true"><GitFork size={13} /></span>
												<span class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap"
													>{hier.caption || hier.name}</span
												>
											</button>
				...
				{#if metadata.dimensions.length === 0}
					<li class="p-2 text-sm text-fg-subtle">{i18n.t('panels.noDimensions')}</li>
				{/if}
			</ul>
		</section>
	{/if}
...
<style>
	...
	.tree__row--level:hover .tree__gear {
		opacity: 1;
	}
	/* saiku#1095: measure rows carry hover actions too (Top N + Sort). */
	.tree__row--measure:hover .tree__gear {
		opacity: 1;
	}
	.tree__gear:hover {
		color: hsl(var(--fg));
	...
	.tree__row--measure .tree__icon--measure {
		color: hsl(var(--primary));
	}
	.tree__row--level {
		color: hsl(var(--fg-muted));
	}
	.tree__row--level .tree__drag {
		cursor: grab;
	}
	.tree__twisty {
		display: inline-flex;
	...
	.tree__node--muted > .tree__row {
		opacity: 0.45;
	}
	.tree__node--muted > .tree {
		opacity: 0.45;
	}
</style>

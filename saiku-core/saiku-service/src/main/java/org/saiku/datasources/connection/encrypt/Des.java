...
public final class Des {
    ...
    private static final int DES_ROUNDS = 16; // Iterations of the convolution

    public static final int DES_BLOCK_BYTES = 8; // 64 bits, to be precise
    /*
     * Bits used in the m_Flags field.
     */

    private static final int DF_KEYSET = 0x01; // Key(s) set.

    private static final int DF_3DES = 0x02; // Triple DES in operation.

    private static final int s_IP[] = {
        0x00000000,
        0x00000001,
        0x00000000,
        0x00000001, /* 00 - 03 */
    ...
    private static final byte s_KP[] = {
        4, 12, 20, 28, 3, 11, 19, 27, 35, 43, 51, 59, 2, 10, /* 00 - 0d */ 18, 26, 34, 42, 50, 58, 1, 9, 17, 25, 33, 41,
        49, 57, /* 0e - 1b */ 36, 44, 52, 60, 5, 13, 21, 29, 37, 45, 53, 61, 6, 14, /* 1c - 29 */ 22, 30, 38, 46, 54,
        62, 7, 15, 23, 31, 39, 47, 55, 63, /* 2a - 37 */
    };

    private static final int DES_NUM_WEAK_KEYS = 64;

    private static final int s_KeyWeak[][] = {
        {0x0000000, 0x0000000},
        {0xfffffff, 0x0000000},
        {0x0000000, 0xfffffff},
        {0xfffffff, 0xfffffff},
    ...
    private static final short s_KRot = 0x7efc;

    private static final byte s_KPerm[] = {
        28, 19, 49, 6, 58, 53, 6, 25, /*
                                       * 0 - 7
                                       */ 18, 52, 6, 60, 21, 27, 61,
        17, /* 8 - 15 */ 51, 59, 24, 6, 48, 20, 26, 56, /* 16 - 23 */ 50, 29, 16, 57, 34, 40, 6, 32, /* 24 - 31 */ 44,
        11, 3, 36, 8, 41, 5, 10, /* 32 - 39 */ 37, 6, 4, 45, 12, 35, 6, 42, /* 40 - 47 */ 2, 6, 33, 9, 0, 43, 13,
        1, /* 48 - 55 */
    };

    private static final int s_SB[][] = {
        {
            0x01010400,
            0x00000000,
            0x00010000,
    ...
    public int SetKey(boolean encrypt, byte[] key) {
        ...
        if (!KeyPermute(keyPermuted, key)) return 0; // A weak key!

        // Encryption key.
        RoundGen(m_keyRoundCrypt0, keyPermuted, encrypt);
    ...
    public int SetKey(boolean encrypt, byte[] key1, byte[] key2) {
        ...
        if (!KeyPermute(keyPermuted, key1)) return 0; // A weak key!
        // Encryption key.
        RoundGen(m_keyRoundCrypt0, keyPermuted, encrypt);

        if (!KeyPermute(keyPermuted, key2)) return 0; // Another weak key.
        // The second is used for the middle "do other" crypt operation.
        RoundGen(m_keyRoundCrypt1, keyPermuted, !encrypt);
    ...
    private int Crypt(byte[] pvTo, byte[] pvFrom) {
        ...
        if ((pvFrom.length % DES_BLOCK_BYTES) != 0) return 0; // Must be a multiple.

        if ((m_Flags & DF_KEYSET) == 0) return 0; // No key - no transform!
    ...
    private void Munge64(byte[] pOut, byte[] pIn) {
        ...
        DoDes(Halves, m_keyRoundCrypt0);
        if ((m_Flags & DF_3DES) != 0) {
            // Need to do it some more!
            DoDes(Halves, m_keyRoundCrypt1);
            DoDes(Halves, m_keyRoundCrypt0);
        }

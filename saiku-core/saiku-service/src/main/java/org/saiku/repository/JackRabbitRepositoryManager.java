...
public class JackRabbitRepositoryManager implements IRepositoryManager {
    ...
    public static synchronized JackRabbitRepositoryManager getJackRabbitRepositoryManager(
        ...
        if (ref == null)
            ...
            ref = new JackRabbitRepositoryManager(
                    config, data, password, oldpassword, defaultRole, sessionRegistry, workspaces);
        return ref;
    }
    ...
    private void login() throws RepositoryException {
        try {
            ...
            if (oldpassword == null) {
                ...
                session = repository.login(new SimpleCredentials("admin", password.toCharArray()));
            } else {
                ...
                try {
                    ...
                    session = repository.login(new SimpleCredentials("admin", oldpassword.toCharArray()));
                }
            }
        }
    ...
    public boolean moveFolder(String user, String folder, String source, String target) throws RepositoryException {
        ...
        if (target == null) {
            // session.getWorkspace().move(root.getPath(),
            // root.getSession().getRootNode().getPath()+"/homes/home:"+user+"/"+folder);
    ...
    public List<DataSource> getAllDataSources() throws RepositoryException {
        ...
        while (node.hasNext()) {
            ...
            InputStream stream = new ByteArrayInputStream(n.getNodes("jcr:content")
                    .nextNode()
                    .getProperty("jcr:data")
                    .getString()
                    .getBytes());
    ...
    private List<IRepositoryObject> getRepoObjects(
            Node files, List<String> fileType, String username, List<String> roles, boolean includeparent) {
                ...
                if (files.getPrimaryNodeType().getName().equals("nt:file")) {
                    if (fileType != null && !filename.contains(FilenameUtils.getExtension(filename))) {
                        ...
                        repoObjects.add(new RepositoryFileObject(
                                filename, "#" + files.getPath(), extension, files.getPath(), acls));
                    }
                }
                    ...
                    if (!nodename.startsWith("jcr:") && !nodename.startsWith("rep:")) {
                        if (acl2.canRead(node, username, roles)) {
                            ...
                            if (node.getPrimaryNodeType().getName().equals("nt:file")) {
                                if (fileType != null
                                        && !fileType.contains(FilenameUtils.getExtension(node.getName()))) {
                                    continue;
                                }
                                String extension = FilenameUtils.getExtension(nodename);

                                repoObjects.add(new RepositoryFileObject(
                                        node.getName(), "#" + node.getPath(), extension, node.getPath(), acls));
                            }
                        ...
                        for (Node file : objects) {
                            // if (!file.isHidden()) {
                            if (!file.getName().startsWith("jcr:")
                                    && !file.getName().startsWith("rep:")) {

                                if (acl2.canRead(file, username, roles)) {
                                    String filename = file.getName();
                                    String relativePath =
                                            file.getPath(); // repo.getName().getRelativeName(file.getName());
                                    ...
                                    if (file.getPrimaryNodeType().getName().equals("nt:saikufiles")) {
                                        if (fileType != null
                                                && !fileType.contains(FilenameUtils.getExtension(filename))) {
                                            continue;
                                        }
                                        String extension = FilenameUtils.getExtension(nodename);
                                    ...
                                    if (file.getPrimaryNodeType().getName().equals("nt:folder")) {
                                        // repoObjects.add(new RepositoryFolderObject(filename, "#" + relativePath,
                                        // relativePath, acls, getRepoObjects(file, fileType, username, roles)));
                                    }
                                    Collections.sort(repoObjects, new Comparator<IRepositoryObject>() {
